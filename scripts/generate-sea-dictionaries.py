"""Pin ICU78 Khmer/Lao words and Unicode sets; pack native radix data."""
...
def language(name,check=False):
    ...
    for span,propertyName,*_ in ucd.records(archive,'Scripts.txt'):
        if propertyName==script:khmer.update(ucd.points(span))
    ...
    handles=khmer&sa
    begin=set(range(0x1780,0x17b4)) if script=='Khmer' else set(range(0xe81,0xeaf))|set(range(0xedc,0xede))|set(range(0xec0,0xec5))
    end=handles-({0x17d2} if script=='Khmer' else set(range(0xec0,0xec5)))
