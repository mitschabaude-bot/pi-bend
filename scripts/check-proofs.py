...
with tempfile.TemporaryDirectory(prefix='proof-gate-', dir=ROOT / 'build') as temporary:
    ...
    extra_mutations = [
        ('sse-default-drops-raw-block', 'packages/ai/src/api/openai-sse-default-hooks.bend', '("From chunk: " ++ raw(lines)) <> Nil{}', 'Nil{}', 'laws/openai-sse-default-hooks.invalid_json_reports_exact_data_and_current_block'),
        ('sse-default-leaks-abort-callback', 'packages/ai/src/api/openai-sse-default-hooks.bend', 'C.dispose(Unit, Unit, cancel)', 'IO.pure(Unit, Unit{})', 'laws/openai-sse-default-hooks.disposal_retires_both_callbacks'),
        ('system-provider-leaks-runtime', 'packages/ai/src/api/openai-responses-system-provider.bend', '        System.dispose(Reason, runtime)', '        IO.pure(Unit, Unit{})', 'laws/openai-responses-system-provider.body_processing_precedes_runtime_retirement_and_preserves_result'),
        ('stream-render-read-cause-lost', 'packages/ai/src/api/openai-responses-stream-error.bend', 'case Stream.ReadFailure{cause}: render(cause)', 'case Stream.ReadFailure{cause}: Done{"lost"}', 'laws/openai-responses-stream-error.read_preserves_callback_diagnostic'),
        ('stream-render-diagnostic-cause-lost', 'packages/ai/src/api/openai-responses-stream-error.bend', 'case SSE.DiagnosticFailure{cause}: render(cause)', 'case SSE.DiagnosticFailure{cause}: Done{"lost"}', 'laws/openai-responses-stream-error.diagnostic_failure_preserves_its_own_cause'),
        ('stream-render-terminal-cause-lost', 'packages/ai/src/api/openai-responses-stream-error.bend', 'case Stream.TerminalFailure{cause}: Done{Terminal.errorMessage(cause)}', 'case Stream.TerminalFailure{cause}: Done{"lost"}', 'laws/openai-responses-stream-error.terminal_message_preserved'),
        ('envelope-render-header-cause-lost', 'packages/ai/src/api/openai-responses-envelope-error.bend', 'case Envelope.HeaderFailure{cause}: header(cause)', 'case Envelope.HeaderFailure{cause}: "lost"', 'laws/openai-responses-envelope-error.header_diagnostic_survives_envelope_wrapping'),
        ('envelope-render-query-position-lost', 'packages/ai/src/api/openai-responses-envelope-error.bend', 'case Address.InvalidQuery{cause}: query(cause)', 'case Address.InvalidQuery{cause}: "lost"', 'laws/openai-responses-envelope-error.query_position_survives_both_boundaries'),
        ...
        ('http-weekday-mismatch-accepted', 'packages/runtime/src/http-date.bend', 'case False{}: Fail{WeekdayMismatch{}}', 'case False{}: Done{tagged(leap, milliseconds)}', 'laws/http-date.mismatched_weekday_never_produces_an_instant'),
        ('http-calendar-cause-lost', 'packages/runtime/src/http-date.bend', 'case Fail{cause}: Fail{CalendarFailure{cause}}', 'case Fail{cause}: Fail{CalendarFailure{Calendar.InvalidYear{}}}', 'laws/http-date.calendar_failure_preserves_cause'),
        ('http-year-cause-lost', 'packages/runtime/src/http-date.bend', 'case _ Fail{cause}: Fail{YearFailure{cause}}', 'case _ Fail{cause}: Fail{YearFailure{Year.InvalidShortYear{}}}', 'laws/http-date.year_failure_preserves_cause'),
        ('preparation-tool-diagnostic-lost', 'packages/ai/src/api/openai-responses-preparation-error.bend', 'case Request.ToolConversion{why}: Sampling.errorMessage(why)', 'case Request.ToolConversion{why}: "tool failure"', 'laws/openai-responses-preparation-error.grammar_diagnostic_is_stage_independent'),
        ('preparation-credential-provider-lost', 'packages/ai/src/api/openai-responses-preparation-error.bend', 'case Preparation.CredentialFailure{why}: Auth.errorMessage(why)', 'case Preparation.CredentialFailure{why}: "No API key"', 'laws/openai-responses-preparation-error.credentials_keep_provider_diagnostic'),
        ('response-cleanup-replaces-primary-diagnostic', 'packages/ai/src/utils/provider-request-error.bend', 'case Request.ResponseFailure{cause, _}: Done{renderOriginal(cause)}', 'case Request.ResponseFailure{cause, Done{_}}: Done{renderOriginal(cause)}\n    case Request.ResponseFailure{_, Fail{cleanup}}: Done{renderOriginal(cleanup)}', 'laws/provider-request-error.response_cleanup_cannot_replace_primary_diagnostic'),
        ('cleartext-runtime-leaks-resolver-on-init-failure', 'packages/ai/src/api/openai-responses-cleartext-runtime.bend', '        Resolver.close(resolver)\n        return Fail{RetryFailure{cause}}', '        return Fail{RetryFailure{cause}}', 'laws/openai-responses-cleartext-runtime.retry_failure_retires_resolver_before_returning_cause'),
        ('cleartext-runtime-omits-fetch-disposal', 'packages/ai/src/api/openai-responses-cleartext-runtime.bend', '        C.dispose(Input.Input<Reason>, Attempt.FetchResult(Reason, Source), fetch)\n', '', 'laws/openai-responses-cleartext-runtime.fetch_retires_before_retry_and_resolver'),
        ('hook-options-drop-timeout', 'packages/ai/src/utils/provider-hook-options.bend', 'key, fetch, env, payload, response, headers, timeout, count, delay}', 'key, fetch, env, payload, response, headers, None{}, count, delay}', 'laws/provider-hook-options.replacement_preserves_all_other_options'),
        ('hook-options-drop-response-hook', 'packages/ai/src/utils/provider-hook-options.bend', 'key, fetch, env, payload, response, headers, timeout, count, delay}', 'key, fetch, env, payload, None{}, headers, timeout, count, delay}', 'laws/provider-hook-options.replacement_installs_exact_hooks'),
        ('prepared-acquisition-loses-base-url', 'packages/ai/src/api/openai-responses-prepared-acquire.bend', '  match result:\n    case Fail{cause}: Fail{cause}\n    case Done{response}: Done{Owner{prepared, response}}', '  match prepared result:\n    case _ Fail{cause}: Fail{cause}\n    case Preparation.Prepared{key, url, headers, payload, compat, grammar, retention} Done{response}: Done{Owner{Preparation.Prepared{key, "", headers, payload, compat, grammar, retention}, response}}', 'laws/openai-responses-prepared-acquire.successful_acquisition_retains_preparation_and_response'),
        ('hook-owner-borrow-drops-hooks', 'packages/ai/src/utils/provider-hook-map.bend', 'case Owner{+hooks}: (Owner{hooks}, hooks)', 'case Owner{+hooks}: (Owner{hooks}, Hooks{None{}, None{}})', 'laws/provider-hook-map.borrowing_preserves_hook_owner'),
        ('hook-owner-does-not-dispose-adapter', 'packages/ai/src/utils/provider-hook-map.bend', 'case Some{value}: C.dispose(Input, Result<&2, &2, E, Value>, value)', 'case Some{value}: IO.pure(Unit, Unit{})', 'laws/provider-hook-map.present_adapter_retirement_disposes_callback'),
        ('hook-owner-skips-payload-retirement', 'packages/ai/src/utils/provider-hook-map.bend', '        retireOptional(T.PayloadInput<Value, Model>, Maybe<&2, Value>, E, payload)\n', '', 'laws/provider-hook-map.both_adapters_retire_in_order'),
        ('default-stream-drops-grammar', 'packages/ai/src/api/openai-responses-stream-defaults.bend', 'Pricing.OpenAIResponsesStreamOptions{requested, Some{grammar}, None{}, Some{callback}}', 'Pricing.OpenAIResponsesStreamOptions{requested, None{}, None{}, Some{callback}}', 'laws/openai-responses-stream-defaults.borrowing_preserves_owner_and_configuration'),
        ('default-stream-omits-pricing-disposal', 'packages/ai/src/api/openai-responses-stream-defaults.bend', 'case Owner{callback}: C.dispose(Pricing.PricingRequest, Result<&2, &2, E, T.Usage>, callback)', 'case Owner{callback}: IO.pure(Unit, Unit{})', 'laws/openai-responses-stream-defaults.disposal_retires_owned_pricing_callback'),
        ('default-session-drops-prepared-grammar', 'packages/ai/src/api/openai-responses-default-session.bend', 'case Preparation.Prepared{_, _, _, _, _, properties, _}: properties', 'case Preparation.Prepared{_, _, _, _, _, properties, _}: R.new(String)', 'laws/openai-responses-default-session.prepared_grammar_is_preserved'),
        ('default-session-omits-dependency-retirement', 'packages/ai/src/api/openai-responses-default-session.bend', '    Defaults.dispose(E, owner)\n    dependencies', '    Defaults.dispose(E, owner)', 'laws/openai-responses-default-session.default_pricing_precedes_dependency_retirement'),
        ('default-session-drops-owned-pricing', 'packages/ai/src/api/openai-responses-default-session.bend', 'Native.Context{url, headers, fetch, retry, pricing, hooks, limit, synthesize}', 'Native.Context{url, headers, fetch, retry, Pricing.defaultOptions(Errors.Error<Reason, SourceError, HookError>), hooks, limit, synthesize}', 'laws/openai-responses-default-session.owned_pricing_reaches_processor_context'),
        ...
        ('native-session-retires-caller-signal', 'packages/ai/src/api/openai-responses-native-session.bend',
         'case Supplied{_}: IO.pure(Unit, Unit{})', 'case Supplied{borrowed}: Abort.dispose(Reason, Abort.AbortController{borrowed})', 'laws/openai-responses-native-session.supplied_signal_retirement_has_no_effects'),
        ('native-session-drops-selected-signal', 'packages/ai/src/api/openai-responses-native-session.bend',
         'T.ProviderRequestOptions{Some{parent}, telemetry, key, fetch, env, payload, response, headers, timeout, count, delay}', 'T.ProviderRequestOptions{None{}, telemetry, key, fetch, env, payload, response, headers, timeout, count, delay}', 'laws/openai-responses-native-session.chosen_signal_reaches_retry_options'),
        ('native-session-drops-response-hook', 'packages/ai/src/api/openai-responses-native-session.bend',
         'T.ProviderRequestOptions{Some{parent}, telemetry, key, fetch, env, payload, response, headers, timeout, count, delay}', 'T.ProviderRequestOptions{Some{parent}, telemetry, key, fetch, env, payload, None{}, headers, timeout, count, delay}', 'laws/openai-responses-native-session.binding_preserves_other_options'),
