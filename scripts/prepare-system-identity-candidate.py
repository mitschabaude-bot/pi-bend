"""Add the platform identity effect to an isolated Bend installation."""
import argparse
...
addition = ROOT / 'patches/experimental/system-identity'
with (args.destination / 'base.bend').open('a') as output:
    output.write((addition / 'base.bend').read_text())
for name in ['system_identity.c', 'system_identity.js']:
    shutil.copyfile(addition / name, args.destination / 'effs' / name)
