"""Prepare additive connection-refusal classification without changing the installation."""
import argparse
...
addition=ROOT/'patches/experimental/socket-refused'
with (args.destination/'base.bend').open('a') as output:output.write((addition/'base.bend').read_text())
for name in ['socket_is_connection_refused.c','socket_is_connection_refused.js']:shutil.copyfile(addition/name,args.destination/'effs'/name)
