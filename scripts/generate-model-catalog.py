...
Usage: scripts/generate-model-catalog.py <data-dir> <provider-id>...
       scripts/generate-model-catalog.py --images <image-models.json>

The image catalog comes from pi-ai's checked-in image-models.generated.ts,
dumped to JSON first:
  node -e 'import(process.env.PI_MONO + "/packages/ai/src/image-models.generated.ts").then(m => console.log(JSON.stringify(m.IMAGE_MODELS)))' > image-models.json
...
def bits(value):
    """The binary64 value by its bit pattern (high word, low word), for
    prices such as 0.0833333333333333 that no small quotient rounds to."""
    import struct
    word = struct.unpack('>Q', struct.pack('>d', value))[0]
    return f'F64{{{word >> 32}, {word & 0xFFFFFFFF}}}'
...
def image_model(value):
    known = ['id', 'name', 'api', 'provider', 'baseUrl', 'thinkingLevelMap', 'input', 'inputLimits', 'cost', 'promptCache', 'headers', 'output']
    exact_keys(value, known, f'image model {value.get("id")}')
    inputs = ordered([enum(INPUTS, 'input')(k) for k in value['input']])
    outputs = ordered([enum(INPUTS, 'output')(k) for k in value['output']])
    return 'T.ImagesModel{' + ', '.join([
        text(value['id']), text(value['name']), text(value['api']), text(value['provider']), text(value['baseUrl']),
        maybe(value.get('thinkingLevelMap'), thinking_map), inputs, maybe(value.get('inputLimits'), input_limits), cost(value['cost']),
        maybe(value.get('promptCache'), prompt_cache), 'None{}', maybe(value.get('headers'), lambda h: record(h, text)), outputs,
    ]) + '}'

def generate_images(data_file):
    """IMAGE_MODELS of pi-ai's image-models.generated.ts, dumped as JSON."""
    catalog = json.loads(pathlib.Path(data_file).read_text())
    lines = ['import Base', 'import ./types.bend as T', 'import ../../runtime/src/record.bend as Record', '',
             '# Generated by scripts/generate-model-catalog.py --images from pi-ai\'s', '# image-models.generated.ts. Do not edit manually.', '']
    providers = []
    for provider, models in catalog.items():
        names = []
        for model_id, value in models.items():
            if value['id'] != model_id or value['provider'] != provider:
                raise ValueError(f'images {provider}: entry {model_id} is {value["provider"]}/{value["id"]}')
            name = 'image_' + identifier(model_id)
            if name in names:
                raise ValueError(f'images {provider}: identifier collision {name}')
            names.append(name)
            lines.append(f'def {name}() -> T.ImagesModel<T.JsonValue>: {image_model(value)}')
            lines.append('')
        constant = provider.upper().replace('-', '_') + '_IMAGE_MODELS'
        lines.append(f'def {constant}() -> List<&2, T.ImagesModel<T.JsonValue>>: ' + ordered([f'{n}()' for n in names]))
        lines.append('')
        providers.append((provider, constant))
    lines.append('# upstream IMAGE_MODELS: each provider\'s models in catalog order.')
    lines.append('def IMAGE_MODELS() -> Record.Record<List<&2, T.ImagesModel<T.JsonValue>>>: Record.Record{' + ordered([f'Record.Property{{{text(p)}, {c}()}}' for p, c in providers]) + '}')
    lines.append('')
    target = ROOT / 'packages/ai/src/image-models.generated.bend'
    target.write_text('\n'.join(lines))
    print(f'{target}: {sum(len(m) for m in catalog.values())} image models')

if __name__ == '__main__':
    if sys.argv[1] == '--images':
        generate_images(sys.argv[2])
        sys.exit(0)
