"""Prepare additive OS interface-index lookup without changing the installation."""
import argparse
...
addition=ROOT/'patches/experimental/interface-index'
with (args.destination/'base.bend').open('a') as output:output.write((addition/'base.bend').read_text())
for name in ['interface_index.c','interface_index.js']:shutil.copyfile(addition/name,args.destination/'effs'/name)
