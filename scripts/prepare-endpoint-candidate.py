"""Prepare additive socket endpoint observation without changing the installation."""
import argparse
...
addition=ROOT/'patches/experimental/socket-endpoint'
with (args.destination/'base.bend').open('a') as output:output.write((addition/'base.bend').read_text())
for name in ['socket_endpoint.c','socket_endpoint.js']:shutil.copyfile(addition/name,args.destination/'effs'/name)
