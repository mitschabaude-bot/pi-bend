"""Prepare additive owned connection attempts without changing the installation."""
import argparse
...
addition=ROOT/'patches/experimental/connect'
with (args.destination/'base.bend').open('a') as output:output.write((addition/'base.bend').read_text())
for name in ['connect.c','connect.js']:shutil.copyfile(addition/name,args.destination/'effs'/name)
