"""Prepare additive OS hostname discovery without changing the installation."""
import argparse
...
addition=ROOT/'patches/experimental/hostname'
with (args.destination/'base.bend').open('a') as output:output.write((addition/'base.bend').read_text())
for name in ['get_hostname.c','get_hostname.js']:shutil.copyfile(addition/name,args.destination/'effs'/name)
