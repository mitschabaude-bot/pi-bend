...
Generic behavioral contracts live in [LAWS.bend](LAWS.bend), with machine-checked implementations in [PROOF.bend](PROOF.bend). Run `bend PROOF.bend` as the proof gate, or `python3 scripts/check-proofs.py` to also check rejection of open obligations and well-typed broken implementations. The current gate checks 268 public laws and 59 supporting lemmas, with 157 typed mutations rejected. [Proof coverage](docs/laws.md) distinguishes these guarantees from remaining agent, IO and runtime work. Differential, integration and performance tests remain complementary.
