...
The modular OpenAI Responses provider runs over native DNS, TLS 1.3, certificate validation, HTTP and SSE. It connects to the library agent loop and public read, write, edit and Bash tools. A live native subagent authored a glob-matching feature, applied review feedback, and completed verification on Bun and native one/four threads. The native Anthropic Messages provider is also registered in the model runtime; local HTTP/SSE checks cover responses, errors, tool-result replay and OAuth refresh, but no live Anthropic service request has been run. [Provenance and validation](tests/glob.md) distinguish these milestones from full port completion.
...
The modular CLI supports noninteractive text/JSON output, an interactive terminal loop, a native JSONL RPC endpoint, model listing, and standalone HTML session export, with OpenAI API-key authentication, stored Codex OAuth credentials, session persistence, and read/write/edit/Bash tools; see [CLI validation](tests/print-cli.md). The interactive loop mounts the editor, streams messages and tool calls, restores saved tool results, updates its footer from session events, switches light/dark themes with terminal color notifications, and restores the terminal on exit. Its worker handles `/thinking` and `/compact` without blocking input; `/model` can display the current model, while switching and new-session commands remain unfinished. Live native runs received an OpenAI answer and executed Bash; one long-lived RPC process handled two model turns and retained their context. RPC now exposes the complete typed state, queue and thinking controls, session-tree reads, session statistics, and lazy HTML export; command coverage remains incomplete. Remaining work includes interactive command and terminal parity, complete provider coverage, resource-loading integration, native extensions, and complete OAuth flows. The prototype's libcurl/ICU adapters remain migration liabilities and are not dependencies of the pure library build.
