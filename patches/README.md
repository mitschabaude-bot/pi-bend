...
`bend-native-f64.patch` (2026-09-22) adds Base's native `F64` (two `U32` words of IEEE 754 bits) with arithmetic, comparison, conversion, shortest decimal spelling and reading as primitives on the host, CUDA and JS lanes (Metal has no `double`; there the operations fail-stop as functions the device does not hold). It is installed in `build/bend-native-toolchain/bend2`; evidence is in the issue log's BEND-013 entries.
