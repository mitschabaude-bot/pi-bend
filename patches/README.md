...
`bend-compiler-literal-memory.patch` is now installed in Bend 2.0.7's `bend.ts` and `comp.ts`. It shares immutable parsed literal-word trees between term representations, omits redundant checked bit annotations after all existing validation, clears the per-definition telescope cache, and releases the preceding emission pass before constructing the next. The clean native toolchain in `build/bend-native-toolchain/bend2` uses the same compiler files with the previously exercised native primitive declarations; `build-pure.sh` and the proof gate select it by default when present. Explicit `BEND` overrides remain supported. The installed launcher retains telemetry and automatic updates. [Measurements and validation](../docs/bend-issues/2026-09-21-literal-sharing.json) show a full-provider peak of 22.4 GiB versus 33.2 GiB for the earlier candidate, byte-identical C, and faster observed compilation. This is an adopted partial improvement, not closure of the memory issue. Additional literal interning remains experimental.
