...
`bend-compiler-literal-memory.patch` is installed in Bend 2.0.7's `bend.ts` and `comp.ts`. It shares immutable literal-word trees between term representations, canonicalizes checked literals after all existing validation, interns words only for the exact same source-span object, clears the per-definition telescope cache, releases the preceding emission pass before constructing the next, and bounds generated indentation to 32 spaces. The clean native toolchain in `build/bend-native-toolchain/bend2` uses the same compiler files with the previously exercised native primitive declarations; `build-pure.sh` and the proof gate select it by default when present. Explicit `BEND` overrides remain supported. The installed launcher retains telemetry and automatic updates. The [first adopted result](../docs/bend-issues/2026-09-21-literal-sharing.json) reduced the full-provider peak from 33.2 to 22.4 GiB; the [follow-up](../docs/bend-issues/2026-09-21-compiler-retained-roots.json) measured 20.7 GiB and 649 seconds versus 668 seconds, with all non-indentation C bytes unchanged. Small-fixture timing differences are mixed and about one hundredth of a second; shared-host measurements are not a universal performance guarantee. The historical incremental interning and cached-prefix patches remain under `experimental/`, but their changes are now included in this installed combined patch. The one-line empty-spine guard is also installed: it avoids unbinding a function type when there are no application arguments. Its [full build](../docs/bend-issues/2026-09-21-constructor-field-cache.json) measured 18.2 GiB and 646 seconds with byte-identical C. The constructor-field cache is now installed as well: it reuses field domains before specialization, with a bounded value key for closed literal word indices and identity for other single arguments. Its full-provider run measured 13.4 GiB and 524 seconds versus 18.2 GiB and 646 seconds, with byte-identical C. The historical incremental patch remains under `experimental/`; the installed combined patch is authoritative. The combined layout cache and early release of non-final output are also installed: [full validation](../docs/bend-issues/2026-09-21-layout-allocation.json) measured 10.2 GiB and 322 seconds versus 13.4 GiB and 524 seconds, with byte-identical C. The original end-pass collection points remain unchanged. The standalone experiment moving collections before passes remains uninstalled. Excessive compiler memory is still an active blocker. The probe table now clears at the same per-definition boundary, retaining only the global erased-variable sentinel. [Validation](../docs/bend-issues/2026-09-21-scoped-probes.json) measured 10.08 GiB sampled peak and 319 seconds, with byte-identical full C and passing captured/concurrent callback, string-fallback and indexed-word runtime checks. This is a modest retention fix; excessive compiler memory remains unresolved. The early-discard change is now installed too: once a pass grows an optimization fact, it stops retaining C text while continuing all analysis. The final stable pass emits normally. [Full validation](../docs/bend-issues/2026-09-21-unstable-output.json) measured 9.74 GiB and 317 seconds with identical C. The incremental experimental patch remains as history; the aggregate patch is authoritative. The [closed-layout cache](../docs/bend-issues/2026-09-21-closed-layouts.json) is also installed: it shares layouts for parameter-free datatypes and recognized word lengths within the existing per-definition lifetime. Full compilation measured 225 seconds/9.48 GiB sampled peak, with identical C and passing dependent-layout regressions. [Literal constant analysis](../docs/bend-issues/2026-09-21-literal-constant-analysis.json) now reuses the existing closed-word marker without changing checking. Full compilation measured 214 seconds/9.19 GiB sampled peak with identical C. Closed parsed strings are now shared after validation too: [full comparison](../docs/bend-issues/2026-09-21-string-literal-sharing.json) measured 204 seconds/8.83 GiB with identical C and passing string/generic/closure runtime checks. This is installed in both compiler paths; excessive memory remains unresolved.
