...
`bend-file-modified.patch` adds `File.modified(path) -> IO(Result<U32 & String, U32 & U32>)`, the modification time in milliseconds since the Unix epoch (high word, low word), following links: `stat` natively, `statSync().mtimeMs` on the JS backend, nanoseconds truncated as Node's `mtime.getTime()`. Session discovery orders files by it and session listings fall back to it. It is installed in the worktree toolchain `build/cli-toolchain` alongside `bend-directory-metadata.patch`, which the print-CLI toolchain had lacked (its `File.kind`/`Directory.read_bytes` declarations were applied by hand after the hunk offset moved).
