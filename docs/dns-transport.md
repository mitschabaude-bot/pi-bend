...
The read-side operation primitive is now implemented in the isolated candidate as `UDPRead.new/wait/cancel/release`. Initial cancellation, completion, stale-handle and socket-reuse tests pass with native resource audits. Next validate packet-arrival races and integrate abort/deadline observation; add corresponding send cancellation and performance evidence before adopting the primitives or assembling the UDP DNS driver.
