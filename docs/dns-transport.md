...
Loopback tests compose this exchange with the prepared attempt-scope lifecycle. Multi-server retry driving, response-code/EDNS fallback policy, TCP fallback and adoption into the system resolver remain unfinished. The TCP resolver remains the live configured resolver.
