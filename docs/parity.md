...
- Per-request API-key resolution now matches 30 original-expression cases on one/four threads. It awaits the resolver with the model provider, treats empty/absent results as fallback requests, reads fallback keys after callback-side rotation and propagates failures without fallback. Tests use dummy values and check invocation counts and retained mutations. Provider dispatch and actual authentication integration remain pending; upstream suite statuses are unchanged.

- Fixed a transcript fidelity gap: raw/normalized context message arrays now use shared cells. Normalization preserves array identity when no initial message is needed, prepending copies only the array, collapse always allocates, and supported transcript resolution retains its array. Existing replay/text/tool assertions and nine direct identity/mutation cases pass on one/four threads. Agent transformation/conversion now composes with canonical normalization while retaining the converter's array. Full array APIs, legacy tool-array identity and provider dispatch remain pending; upstream suite statuses are unchanged.

- Request configuration snapshots now follow awaited key resolution and retain all typed loop fields. Sixty source-expression cases pass on one/four threads, checking fresh snapshot identity, shared model/converter fields, explicit absent/present signal overrides, original-state preservation and key rotation. Complete stream handoff must still retain agent-only spread fields beyond the existing provider-options type; dispatch remains pending and upstream suite statuses are unchanged.

- Assistant content arrays now use shared cells, and the agent has a typed message-snapshot primitive. One/four-thread tests verify fresh top-level identity, visible content-array/object mutations and independent field replacement. Existing assistant-event-stream, agent context/event and transcript assertions retain their expectations with explicit content-array ownership. Other nested object/array identities and the streamed-response consumer remain pending; upstream suite statuses are unchanged.

- Assistant usage records and diagnostics arrays/entries now retain shared identities. Snapshot tests on one/four threads verify mutation visibility and independence from subsequent top-level field replacement. Stream, transcript and agent fixtures retain their existing assertions with explicit ownership of usage cells. Remaining nested records, tool/image usage identity and the streamed-response consumer are unfinished; upstream suite statuses are unchanged.

- The canonical assistant-response consumer now handles start, all nine updates, terminal done/error and iterator exhaustion, with awaited emissions and current-array transcript writes. Twenty-seven comparisons against the actual source loop pass on one/four threads, including missing/repeated starts, listener failures and listener-side transcript edits. Snapshot/live-final identity and explicit snapshot/iterator lifetimes are tested. Provider dispatch, full agent-loop integration, dynamic array properties and global Promise scheduling remain pending; upstream suite statuses are unchanged.

- The typed assistant-request path now composes transform/conversion, canonical normalization, key resolution, full option snapshot, awaited injected stream creation and response consumption. All 27 response traces pass directly and through the composed path on one/four threads, with provider/input/config identity, invocation counts and explicit allocation ownership checks; conversion failure skips dispatch and allocation. `StreamFn` makes its concrete options record explicit to preserve the agent's full spread without casts. Actual providers/networking, complete loop integration and dynamic semantics remain pending; upstream suite statuses are unchanged.

- Post-response completion now appends the shared assistant message and handles error/aborted termination with awaited turn-end then agent-end events. Twenty-one actual-source cases pass on one/four threads, covering all stop reasons, listener failures, live mutations, message/array identity and allocation ownership. Normal tool handling and full loop integration remain pending; upstream suite statuses are unchanged.

- Token-limit tool failures now compose canonical start/end emission, fresh error details, timestamped message construction and message-start/end before batch append. Thirty-seven comparisons against the original batch/helper bodies pass on one/four threads, including listener failures, post-callback renaming and live-array append iteration. Returned batches never terminate. Normal tool execution, heterogeneous detail integration and the complete outer loop remain pending; upstream suite statuses are unchanged.

- Normal tool preparation now combines canonical content filtering with whole-batch execution-mode selection. Eighty-four comparisons with the original dispatcher pass on one/four threads, checking order/duplicates, first-definition overrides, all global modes, fresh filtered arrays and live shared calls after original-array mutation. Selection cannot invoke execution or conversion callbacks. Actual execution/validation and the complete outer loop remain pending; upstream suite statuses are unchanged.

- Normal preparation now has typed prepared/immediate outcome contracts preserving shared call/tool/result identity and distinct validated arguments. Source field checks, native discriminator/identity tests and a negative type check pass. Before-hook decisions match 74 original-source cases on one/four threads, including abort precedence, empty/default reasons and termination flags. Hook invocation, validation, immediate-result construction and the complete execution pipeline remain pending; upstream suite statuses are unchanged.

- Added canonical validation schema-type filtering and value-type predicates, backed by exact pure-Bend binary64 integer classification. Tests compare 2,830 predicates and 16 schema-property cases against the actual original helper bodies, plus an absent-property check, on one/four threads. Non-JSON variants stay typed instead of being serialized away. Full TypeBox compilation/coercion, live-object semantics and public validation entry points remain pending; upstream suite statuses are unchanged.

- Added exact pure-Bend rational-to-binary64 conversion as a dependency for validation's numeric-string coercion. All 226 independent exact-rational cases pass on one/four threads, including signed zero, midpoint ties, subnormal transitions, overflow and large operands without intermediate floating-point rounding. Numeric text parsing and full validation integration remain pending; upstream suite statuses are unchanged.

- Added pure-Bend numeric-string conversion for validation's upcoming primitive coercion. All 365 comparisons with JavaScript `Number` pass on one/four threads, covering full-string syntax, whitespace, signed zeros, radix forms, long exponents and exact rounding boundaries. Mantissas use arbitrary-size integers until final rational rounding. Schema coercion, TypeBox compilation and the public validation path remain unfinished; upstream suite statuses are unchanged.

- Canonical primitive schema coercion now composes pure-Bend parsing, numeric classification and shortest formatting. All 860 comparisons with the original helper pass on one/four threads, checking accepted conversions and exact preservation of rejected/non-JSON values. An explicit formatting-failure result prevents silent fallback if the numeric dependency fails. Recursive schema coercion, TypeBox validation and tool integration remain unfinished; upstream suite statuses are unchanged.
