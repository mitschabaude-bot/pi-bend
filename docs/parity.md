...
- `packages/ai/src/types.bend` starts the typed library port with faithful content, usage, model-cost records and stop reasons. Numeric fields use binary64, and optional values remain distinct. The module is partial; the full Model/schema/options contracts and transcript replay remain unported.
...
- Raw/normalized contexts, system messages and generic tool declarations now have canonical types. Initial context normalization and text/prompt rendering pass two of nine upstream replay-suite cases, plus native separator/ordering/identity regressions. The supporting own-property record passes 50 JavaScript differential mutation sequences. Complete replay, declaration comparison, schema validation and custom-role integration remain pending.
