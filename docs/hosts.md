...
Bounded file loading is implemented below. The explicit files-first dispatch policy and family filtering are described below; system NSS configuration remains pending, and the native single-family adapter is described below. No production host-resolution path has been changed by adding this pure core.
...
Four generic laws cover chunk composition, sticky overflow, exclusion of excess bytes and single-byte budget consumption for arbitrary values and retained prefixes. Live file and pipe tests check error precedence and closure. The resolver's existing `readWith`/`loadWith` entry points and error constructors remain unchanged; their internal budget state moved into the reusable core. System source configuration remains pending.
...
## Native single-family DNS adapter

`dns-hosts-resolver.lookupWith` now connects the proved files-first plan to a borrowed `dns-resolver.Resolver` and parent abort signal. Its family type requires an explicit IPv4 or IPv6 choice, selecting A or AAAA respectively. It returns complete hosts rows, a source error, a DNS presentation error, or the native DNS report including its retained deadline reason. `lookup` supplies the standard random identifier source. Neither entry point owns or closes the persistent resolver.

Local matches and source errors bypass DNS parsing, identifier generation, server selection and deadline creation. On a miss, `dns-search-text` parses presentation syntax before preparing the search input. It retains the parsed wire name and absolute flag and counts presentation dots for the configured `ndots` threshold, matching the counting rule in pinned [glibc 2.39 resolver search](https://raw.githubusercontent.com/bminor/glibc/glibc-2.39/resolv/res_query.c). This includes literal dots inside escaped presentation input; the existing presentation parser determines label boundaries and the absolute flag separately. Accepted presentation input is bounded by the wire-name grammar before its natural-number dot count is converted to the existing U32 search API.

The adapter preserves the native search loop's suffix, alias, retry, UDP/TCP and error policy. It does not yet supply combined-family scheduling, numeric-host short-circuiting, IDNA normalization, system source configuration/reloading or HTTP integration. Those remain distinct work toward full hostname resolution.
