...
Grammar schema inspection uses native JSON-schema validation rather than JavaScript object access/coercion: a null root reports the typed object-schema requirement instead of an engine TypeError, and a present non-object property schema reports the string-schema requirement instead of being treated as absent by JavaScript falsiness. A properties value must be an object map; arrays are not property maps. These malformed-input diagnostic/shape distinctions are excluded from claims of exact source equivalence. Supported grammar declarations and all four ported constrained-sampling tests retain upstream behavior.

Gregor approved clearing SSE diagnostic lines at each empty block (2026-09-19). The pinned SDK retains comment/unknown-field lines across heartbeat blocks until a data/event-bearing block, allowing indefinite accumulation. Bend should retain only the current block for diagnostics while preserving event names, data and event emission behavior.

### Approved: malformed resolver option policy

Linux glibc 2.39 accepts prefix matches and partial integer conversion in resolver options: `timeout:7junk` becomes 7, `ndots:-1` becomes 15, an empty `attempts:` becomes 0, and `rotate-junk` enables rotation. Large numeric values can also wrap during integer conversion. These were observed through the test-only `res_ninit` oracle in `tests/resolver-options-oracle.c`; no queries are sent. The [glibc parser](https://raw.githubusercontent.com/bminor/glibc/glibc-2.39/resolv/res_init.c) explains the behavior.

Gregor approved rejecting malformed configuration with diagnostics on 2026-09-20, explicitly preferring rejection over permissive parsing. Recognize whole option names and unsigned decimal values, apply documented numeric caps without integer wrapping, and reject malformed/unknown-option reports at configuration construction rather than silently using partial values. The existing explicitly strict APIs provide this behavior; application wiring remains implementation work. Older pending-policy notes are superseded by this decision.

### Approved: broader native correctness policy (2026-09-20)

Gregor accepted the recommendations summarized in the pause review and explicitly extended the instruction against incidental JavaScript semantics to obvious flaws in legacy software. Preserve pi's meaningful contracts and valid-input behavior; do not reproduce dependency bugs or permissive malformed-input accidents merely for reference equality. Reject invalid cases with useful typed diagnostics. Intentional departures must stay explicit in differential tests rather than being counted as exact reference matches.

The reviewed decisions are approved: boolean JSON schemas use their normal schema semantics instead of reproducing an upstream cache failure; declared object property names are literal rather than regexp conversion patterns; internationalized hostnames follow Unicode validation where Node incorrectly accepts invalid names; malformed resolver configuration is rejected as described above. The older malformed-form-decoding note needs its exact case recovered before claiming a specific behavior is implemented; apply the broader policy rather than treating that stale note as an unanswered user question. These approvals do not claim that implementation or integration is complete. Historical pending notes elsewhere are superseded for these reviewed choices.
