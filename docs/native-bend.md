...
### Pending: malformed resolver option policy
...
Proposed native policy: recognize whole option names and unsigned decimal values, apply the documented numeric caps without integer wrapping, and ignore malformed tokens while retaining diagnostics. Alternatives presented to Gregor are rejecting malformed configuration or retaining libc's permissive conversion. The choice is pending. `resolver-options.parseStrict` and `interpretStrict` expose the proposed parser explicitly and retain diagnostics, but no runtime entry point adopts its unresolved reports. Valid-option defaults, typed values and ordered updates are implemented independently of this decision.
