...
[Four generic laws](proof-validation/2026-09-21-http-date-year-standalone.json), with three supporting lemmas, establish literal-year behavior independent of the current clock, weekday independence during century selection, and preservation of invalid-reference causes. They have no unsafe annotations and remain outside the root gate. These laws do not establish the complete rolling-window arithmetic.

The [runtime record](runtime-validation/2026-09-21-http-date-year.json) checks 7,224 cases on native one/four threads and Bun. Its independent oracle enumerates candidate centuries instead of repeating the implementation's division and rollback calculation. Coverage includes every two-digit year around the exact anniversary for seventeen reference years, leap-day anniversaries, random reference dates, calendar endpoints, literal years and invalid references. This is finite evidence, complementary to the laws.

Full HTTP-date interpretation, weekday checking, leap-second representation and installation of the default retry parser remain pending. In particular, the RFC explicitly allows leap seconds; a complete interpreter must address them rather than treating every `23:59:60` as malformed.
