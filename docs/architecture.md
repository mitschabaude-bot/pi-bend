...
The runtime schema-data projection now explicitly models undefined, callable and symbol values plus property enumerability. Its finite-tree snapshot preserves JSON round-trip rules without decimal parsing, since shortest round-tripping preserves finite binary64 values except negative zero. It rejects unsupported callable `toJSON` hooks instead of erasing them. Live-object semantics, TypeBox schema relationships and declaration integration remain pending. Constrained-sampling configuration must also preserve property insertion order before JSON-based declaration equality is faithful; the initial fixed records do not yet retain it.
