...
`api/openai-responses-envelope.bend` combines native Responses URL preparation, JSON serialization and OpenAI header assembly into an immutable unframed request. Its request value retains the typed URL, lowercase `post` method, normalized headers, serialized body and effective timeout. Custom fetch can inspect that value; native HTTP can subsequently normalize the method, derive Host, encode UTF-8 and frame the body. No socket, timer, callback or signal owner is acquired by this pure component.
