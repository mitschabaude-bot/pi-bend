...
Ported: `agent_start`, `agent_end`, `agent_settled`, `turn_start` (with the run's turn index), `message_start`/`message_update`/`message_end` and `tool_execution_*` reach extensions before the session's listeners (`AgentSession` `handleEvent`). `session_before_tree` handlers may cancel a navigation (which then reports `cancelled`), supply the branch summary or override its instructions and label; `session_tree` follows the navigation. `session_before_compact` handlers may cancel a manual, threshold or overflow compaction or supply its result; `session_compact` follows a recorded compaction and `session_compact_failed` a failed or cancelled one. `message_end` handlers may replace the finalized message with one of the same role (`emitMessageEnd`); the listeners, the session entry and the agent's context carry the replacement. `tool_call` handlers run in the agent's `beforeToolCall` (first block wins, a failing handler blocks the call) and `tool_result` handlers patch the result in `afterToolCall` before image normalization (`emitToolResult`). `input` handlers run in `prompt()` after the extension-command and compaction checks and for queued steer/follow-up input (`emitInput`: transform chains, handled stops; the source is interactive, rpc or extension). `pi.sendMessage` and `pi.sendUserMessage` (with `deliverAs` and `expandPromptTemplates`) are bound by the session (`ExtensionActions`); before binding they fail with upstream's "not initialized" message. Deliveries that queue or append happen at once; one that starts a turn runs on its own task, and failures become `<runtime>` extension errors.

`turn_end` (from the agent's finishTurn, or with the turn_end event for a synthesized failure) and `agent_before_settle` run as boundaries: handlers see drafts, the continuation decision and a preview of the context the drafts produce; the drafts (custom, custom_message, context_edit, compaction entries) are committed and announced after the context refresh, and a continuation without runnable context is reported. `context` and `context_with_system` handlers transform every request's messages (the session installs the agent's transformContext): context handlers see the conversation without system messages and a changed result gets the replayed prompt and tool state as one leading system message; context_with_system output is used as returned, with a dropped leading system message reported. Handlers return messages; they cannot edit `event.messages` in place, and "unchanged" is structural equality. Pending: `session_start`, `session_before_switch`/`fork`, `session_shutdown`, `session_info_changed`, `before_agent_start`, `input`, `user_bash`, provider request/response events, `model_select`/`thinking_level_select`, `appendEntry` and the other action methods, the `_isEmittingAgentSettled` deferral of prompts sent from `agent_settled` handlers, and upstream's in-place replacement of the original message in `turn_end`/`agent_end` payloads.
