...
if not args.no_build:
    subprocess.run(['python3','scripts/run-rss-guarded.py','--limit-gib',str(args.build_limit_gib),'--stats','build/dns-tcp-query-build.json','--','sh','scripts/build-pure.sh','tests/dns-tcp-query.bend','build/dns-tcp-query'],cwd=ROOT,env=dict(os.environ,BEND=str(launcher)),check=True)
