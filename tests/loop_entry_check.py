...
cases = [dict(case, entry=entry) for entry in [1, 2] for case in cases]
for entry in [1, 2, 3, 4]:
    for fail in ['', 'agent_start', 'turn_start', 'message_start:prompt', 'message_end:prompt']:
        cases.append(dict(entry=entry, kind=4, terminate=False, steering=False, follow=False, stop=False, failAt=fail))
...
for index, (case, result) in enumerate(zip(cases, expected, strict=True)):
    args = [str(case['entry']), str(case['kind']), flag(case['terminate']), flag(case['steering']),
    ...
    lines.append(f'    IO.print("loop entry case {index}")')
    lines.append('    T.scenarioMode(' + ', '.join(args) + ')')
lines.append(f'    IO.print("PASS {len(cases)} public loop-entry source comparisons")')
