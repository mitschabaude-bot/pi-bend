...
## CLI integration (2026-09-22, print mode)

`main.bend` loads a startup manager (`Settings.create(cwd, agentDir, options())`) and, after the session is chosen, a runtime manager for the session's cwd; load errors print as upstream's `collectSettingsDiagnostics` warnings, each message once. The settings supply the session directory fallback (`getSessionDir`), the saved default provider/model for initial model selection when its provider has credentials (upstream `findInitialModel` step 3), the default and per-model thinking levels (SDK order: CLI, resumed session, per-model, default, off; clamped), `defaultTools` (else read/bash/edit/write), `blockImages` (images become "Image reading is disabled." placeholders in the LLM conversion), and the Agent options `steeringMode`, `followUpMode`, `transport`, `thinkingBudgets` and `maxRetryDelayMs`, with the session id. Not wired yet: project trust (project settings load as trusted), `--models`/`enabledModels` scoping, provider retry `timeoutMs`/`maxRetries`, `httpIdleTimeoutMs`/`websocketConnectTimeoutMs` and `httpProxy` (the native provider keeps its own defaults), theme and terminal capability overrides (interactive), `imageAutoResize`, and the first-time setup. `tests/print_cli_check.py` checks the invalid-settings warning offline.
