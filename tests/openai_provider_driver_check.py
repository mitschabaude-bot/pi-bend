"""Outer OpenAI Responses driver with HTTP/SSE, request hooks and concurrent canonical stream consumers."""
import argparse
import hashlib
import json
import os
...
SOURCE = 'tests/openai-provider-driver.bend'
prefix = WORK / 'build/openai-provider-driver'
...
for audited in [False,True]:
    ...
    for backend,command in [('native-1',[str(prefix)+suffix,'--threads','1']),('native-4',[str(prefix)+suffix,'--threads','4']),('bun',[BUN,str(prefix)+suffix+'.js'])]:
        ...
        for case,original in zip(cases,reference['results'],strict=True):
            mode=case['mode'];trace=list(original['trace']);retained=list(original['retained']);adaptations=[]
            if mode in [3,4]:
                at=next(i for i,line in enumerate(trace) if line.startswith('emit:error:'))
                trace.insert(at,'release');adaptations.append('Release unread acquired response on response-hook/Start failure.')
            if mode==6:
                trace=[line for line in trace if not line.startswith('emit:error:')]
                retained=[line for line in retained if not line.startswith('retained:error:')]
                adaptations.append('Terminal delivery failure closes and settles the typed result without a second emission attempt.')
            if mode==7:
                assert original['unhandled']=='sink' and 'close' not in trace
                trace.append('close');adaptations.append('Rejected error emission closes the stream; upstream rejects without ending it.')
            else:assert original['unhandled'] is None
            cleanup='cleanup' if mode in [11,12] else 'none'
            delivery='sink' if mode in [6,7] else 'none'
            error=original['error'] or 'ok'
            if mode in [11,12]:adaptations.append('The fixture injects a cleanup failure after actual owner retirement; retain it separately from the primary error.')
            wanted=trace+[f'run:{error}:cleanup:{cleanup}:delivery:{delivery}']+retained+[original['final']]
            errors,peers=[],[]
            ...
            if actual!=wanted:
                (ROOT/'build/openai-provider-driver-mismatch.json').write_text(json.dumps({'backend':backend,'mode':mode,'actual':actual,'expected':wanted},indent=2)+'\n')
        ...
        print(backend,'audit' if audited else 'production',len(cases),'outer provider cases PASS',flush=True)
...
changed={'packages/ai/src/api/openai-responses-lifecycle.bend','packages/ai/src/api/openai-responses-driver.bend',SOURCE,'tests/provider-request.bend'}
for path in visited:
...
record={
 'scope':'Generic outer provider driver composed with canonical request hooks/retries, real HTTP uploads and SSE processing, and concurrent canonical stream iterator/result consumers. Actual pinned outer-wrapper/processor oracle with explicit cleanup/delivery adaptations. Test-selected string presentation of typed internal causes; no full serialized provider/auth/TLS/TUI claim.',
...
(ROOT/'build/openai-provider-driver-results.json').write_text(json.dumps(record,indent=2)+'\n')
