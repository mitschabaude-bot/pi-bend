...
The system-message-replay suite is partial: its rendering and legacy-context normalization cases are ported, while seven replay/declaration cases remain pending. `sh tests/transcript.sh` runs the two original cases plus native text edge cases. `python3 tests/record_vectors.py` uses JavaScript only as an oracle for own-property ordering and mutation. The opaque schema fixture preserves the normalization input; it does not stand in for a completed TypeBox port.
