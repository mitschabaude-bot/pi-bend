...
The system-message-replay suite is partial: eight normalization/rendering/replay/collapse/change-detection/history cases are ported, while extended-tool declaration comparison remains incomplete. `sh tests/transcript.sh` runs the eight original cases plus native text and transport-tool edge cases. `python3 tests/record_vectors.py` uses JavaScript only as an oracle for own-property and insertion-ordered Map mutation. The replay fixtures use the verified TypeBox 1.3.27 empty-object data shape; the schema builders and validators remain unported.
