...
The system-message-replay suite is partial: seven normalization/rendering/replay/collapse/change-detection cases are ported, while extended-tool declaration comparison and the combined redefinition/non-additive case remain incomplete. `sh tests/transcript.sh` runs the seven original cases plus native text and transport-tool edge cases. `python3 tests/record_vectors.py` uses JavaScript only as an oracle for own-property and insertion-ordered Map mutation. The opaque schema fixture preserves the normalization input; it does not stand in for a completed TypeBox port.
