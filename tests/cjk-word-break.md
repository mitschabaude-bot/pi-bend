...
The dynamic program preserves ICU's match order, strict-less cost updates, 20-scalar search bound, cost-255 single-character fallback (except Hangul), Katakana cost table, and the strict run-length `<20` cutoff. A rolling 21-slot immutable window suffices because every edge is bounded by 20 scalars. Winning candidates share persistent predecessor paths; no full-length mutable score table or quadratic prefix rescans are required.
