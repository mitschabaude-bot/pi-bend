"""Full pinned Khmer prefix contract, independent source-trie expectations."""
...
words=sorted({line.split('#')[0].strip() for line in (ROOT/'build/icu78-source/khmerdict.txt').read_text(encoding='utf-8-sig').splitlines() if line.split('#')[0].strip()})
trie={}
for word in words:
 node=trie
...
cases+= [word+'ក' for word in words]
cases+= [''.join(random.choices([chr(cp) for cp in range(0x1780,0x17e0)],k=random.randrange(1,32))) for _ in range(4096)]
cases+= ['a','😀','ក'*100000,'\u200d','\u200c']
expected=[]
for text in cases:
...
fixture=ROOT/'build/khmer-prefix-fixture.tsv';fixture.write_text('\n'.join(text+'\t'+json.dumps(want,separators=(',',':')) for text,want in zip(cases,expected)))
for backend in a.backends:
 cmd=['bun','build/khmer-word-break.js'] if backend=='bun' else ['build/khmer-word-break','--threads',backend[-1]]
 ...
 print(f'{backend}: {len(cases)} exact Khmer prefix/candidate results pass ({time.monotonic()-start:.3f}s)',flush=True)
