"""Full pinned Khmer/Lao prefix contract, independent source-trie expectations."""
...
for language in ['khmer','lao']:
 ...
 cases+= [word+('ក' if language=='khmer' else 'ກ') for word in words]
 cases+= [''.join(random.choices([chr(cp) for cp in (range(0x1780,0x17e0) if language=='khmer' else range(0xe80,0xee0))],k=random.randrange(1,32))) for _ in range(4096)]
 cases+= ['a','😀','ក'*100000,'\u200d','\u200c']
 expected=[]
 for text in cases:
