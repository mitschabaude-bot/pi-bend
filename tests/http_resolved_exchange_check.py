"""Real HTTP request/response exchange through numeric, hosts and DNS resolution.
...
prefix = ROOT / 'build/http-resolved-exchange'

def replace_once(source, before, after):
    assert source.count(before) == 1, before
    return source.replace(before, after)

if args.reuse_programs:
    ...
    if not args.no_build:
        for suffix in ['c', 'js']:
            with Path(f'{prefix}-{suffix}.log').open('w') as log:
                subprocess.run(['python3', 'scripts/run-rss-guarded.py', '--limit-gib', str(args.rss_limit_gib), '--stats', f'{prefix}-{suffix}-build.json', '--', str(bun), str(candidate / 'main.ts'), 'tests/http-resolved-exchange.bend', '-o', f'{prefix}.{suffix}'], cwd=ROOT, check=True, stdout=log, stderr=subprocess.STDOUT)
...
def execute(backend, command, audited, arguments, expected_timers, expected_peak=None, flow=0):
    wire_arguments = [*arguments, 9] if len(arguments) == 3 else arguments
    result = subprocess.run([*command, *map(str, [*wire_arguments, flow])], cwd=ROOT, capture_output=True, text=True, timeout=20)
    ...
    results.append(dict(backend=backend, audited=audited, arguments=arguments, flow=flow, resources=values))
...
for audited in [False, True]:
    ...
    for backend, command in [('native 1', [f'{prefix}{suffix}', '--threads', '1']), ('native 4', [f'{prefix}{suffix}', '--threads', '4']), ('Bun', [str(bun), f'{prefix}{suffix}.js'])]:
        for first_family in [4, 6]:
            ...
            for mode in [0, 1, 4]:
                ...
                for scenario in scenarios:
                    with socket.socket(first_af, socket.SOCK_STREAM) as first, socket.socket(second_af, socket.SOCK_STREAM) as second, ThreadPoolExecutor(max_workers=2) as pool:
                        ...
                        if scenario != 'exhausted':
                            ...
                            def serve():
                                with listener.accept()[0] as peer:
                                    ...
                                    wire=b''
                                    while b'\r\n\r\n' not in wire:
                                        part=peer.recv(4096); assert part; wire+=part
                                    head,body=wire.split(b'\r\n\r\n',1)
                                    first_line,*fields=head.split(b'\r\n')
                                    assert first_line==b'POST /resolved?x=1 HTTP/1.1', first_line
                                    headers=dict(field.split(b': ',1) for field in fields)
                                    hostname=('127.0.0.1' if first_family==4 else '[::1]') if mode==0 else 'unit'
                                    assert headers[b'host']==f'{hostname}:{port}'.encode(),headers
                                    assert headers[b'content-length']==b'5',headers
                                    while len(body)<5:
                                        part=peer.recv(1024);assert part;body+=part
                                    assert body==b'hello',body
                                    peer.sendall(b'HTTP/1.1 200 OK\r\nContent-Length: 2\r\nConnection: close\r\n\r\nOK')
                                    assert peer.recv(1)==b'', 'exchange retained socket'
                        ...
                        expected_result = 'connection:exhausted' if scenario == 'exhausted' else 'response:ok'
            ...
            assert execute(backend, command, audited, [2, first_family, 0], 0)==['source-error','unresolved']
            assert execute(backend, command, audited, [3, first_family, 0], 0)==['guard:tls']
            assert execute(backend, command, audited, [5, first_family, 0], 0)==['guard:zero']
        for family,af,host_name in [(4,socket.AF_INET,'127.0.0.1'),(6,socket.AF_INET6,'::1')]:
            for mode in [0,1]:
                label='numeric' if mode==0 else 'hosts'
                assert execute(backend,command,audited,[mode,family,0],0,flow=2)==[label,'connection:abort:stop']
                with socket.socket(af,socket.SOCK_STREAM) as listener,ThreadPoolExecutor(max_workers=1) as pool:
                    listener.bind((host_name,0));listener.listen(1);listener.settimeout(15)
                    port=listener.getsockname()[1]
                    def serve_abort():
                        with listener.accept()[0] as peer:
                            peer.settimeout(10);wire=b''
                            while b'\r\n\r\n' not in wire:
                                part=peer.recv(4096);assert part;wire+=part
                            head,body=wire.split(b'\r\n\r\n',1)
                            first_line,*fields=head.split(b'\r\n')
                            assert first_line==b'POST /resolved?x=1 HTTP/1.1'
                            headers=dict(field.split(b': ',1) for field in fields)
                            hostname=('127.0.0.1' if family==4 else '[::1]') if mode==0 else 'unit'
                            assert headers[b'host']==f'{hostname}:{port}'.encode() and headers[b'content-length']==b'5'
                            while len(body)<5:
                                part=peer.recv(1024);assert part;body+=part
                            assert body==b'hello'
                            peer.sendall(b'HTTP/1.1 200 OK\r\nContent-Length: 2\r\nConnection: close\r\n\r\n')
                            assert peer.recv(1)==b'', 'aborted exchange retained socket'
                    future=pool.submit(serve_abort)
                    output=execute(backend,command,audited,[mode,family,port],int(mode==1),flow=1)
                    future.result(timeout=20)
                    address='2130706433' if family==4 else 'ipv6'
                    attempt_mode='final' if mode==0 else 'timed'
                    assert output==[label,f'ENTRY {address} {attempt_mode} success none','response:ok'],output
...
sources = set()
closure(ROOT / 'tests/http-resolved-exchange.bend', sources)
