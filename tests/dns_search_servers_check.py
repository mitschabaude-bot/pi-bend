"""Search suffixes composed with multi-server address/alias lookup and recovery."""
import argparse
...
if not a.no_build:
    for suffix in ['c','js']:
        subprocess.run(['python3','scripts/run-rss-guarded.py','--limit-gib','16','--stats',f'build/dns-search-servers-{suffix}-build.json','--',str(bun),str(candidate/'main.ts'),'tests/dns-search-servers.bend','-o',f'build/dns-search-servers.{suffix}'],cwd=ROOT,check=True)
subprocess.run(['flock', '/tmp/pi-bend-build.lock', 'clang','-std=c11','-fbracket-depth=2048','-O1','build/dns-search-servers.c','-lpthread','-lm','-o','build/dns-search-servers'],cwd=ROOT,check=True)
...
cases=[
 dict(name='suffix-failover',plan=[(0,'a.x',0,'eof'),(1,'a.x',0,3),(0,'a.y',65535,'eof'),(1,'a.y',65535,'address')],answer='a.y',draws=2),
 dict(name='suffix-alias-failover',plan=[(0,'a.x',0,'eof'),(1,'a.x',0,3),(0,'a.y',65535,'alias:b'),(0,'b',4660,'eof'),(1,'b',4660,'address')],answer='b',draws=3),
 dict(name='reset-next-suffix',plan=[(0,'a.x',0,'reset'),(0,'a.x',0,3),(0,'a.y',65535,'address')],answer='a.y',draws=2),
 dict(name='dns-refused-final',plan=[(0,'a.x',0,5),(0,'a',65535,'eof'),(1,'a',65535,'address')],answer='a',draws=2),
 dict(name='last-connection-refused',servers=['listen','refused'],plan=[(0,'a.x',0,'eof')],dns=2,draws=1),
 dict(name='transport-final',plan=[(0,'a.x',0,'eof'),(1,'a.x',0,'eof'),(0,'a',65535,'eof'),(1,'a',65535,'address')],answer='a',draws=2),
 dict(name='no-data-precedence',plan=[(0,'a.x',0,0),(0,'a.y',65535,3),(0,'a',4660,3)],dns=4,draws=3),
 dict(name='initial-precedence',text='a.b',ndots=1,plan=[(0,'a.b',0,'eof'),(1,'a.b',0,3),(0,'a.b.x',65535,0),(0,'a.b.y',4660,2)],dns=1,draws=3),
 dict(name='all-refused',servers=['refused','refused'],plan=[],dns=2,draws=1),
 dict(name='source-first',mode='error-first',plan=[],error='entropy:11:test source unavailable',draws=1),
 dict(name='source-second',mode='error-second',plan=[(0,'a.x',0,3)],error='entropy:38:test source unavailable',draws=2),
 dict(name='pre-abort',mode='pre',plan=[],error='connect:parent',reason='parent',draws=0),
 dict(name='deadline-suffix-failover',mode='deadline',slow_first=True,plan=[(0,'a.x',0,3),(0,'a.y',65535,'eof'),(1,'a.y',65535,'stall')],error='read:expiry',reason='expiry',draws=2),
 dict(name='deadline-alias-recovery',mode='deadline',slow_first=True,plan=[(0,'a.x',0,'alias:b'),(0,'b',65535,'reset'),(0,'b',65535,'stall')],error='read:expiry',reason='expiry',draws=2),
 ...
 dict(name='alias-loop',plan=[(0,'a.x',0,'alias:b'),(0,'b',65535,'alias:a.x')],error='loop',draws=2),
 dict(name='no-domains',domains='',plan=[(0,'a',0,'eof'),(1,'a',0,'address')],answer='a',draws=1),
 dict(name='duplicate',order=[0,0],plan=[(0,'a.x',0,'eof'),(0,'a.x',0,3),(0,'a.y',65535,'address')],answer='a.y',draws=2),
 dict(name='invalid-name',text='a..b',ndots=1,plan=[],dns=3,draws=0),
 dict(name='absolute',text='a.',plan=[(0,'a',0,'eof'),(1,'a',0,'address')],answer='a',draws=1),
]

rows=[]
for backend,command in [('native 1',['build/dns-search-servers','--threads','1']),('native 4',['build/dns-search-servers','--threads','4']),('Bun',[str(bun),'build/dns-search-servers.js'])]:
    for number,family,host in [(4,socket.AF_INET,'127.0.0.1'),(6,socket.AF_INET6,'::1')]:
        for kind in [1,28]:
            for case_index,case in enumerate(cases):
                ...
                want += [case.get('reason','none'),'draws:'+str(case['draws'])]
