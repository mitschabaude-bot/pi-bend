...
Both suites pass Bun and native one/four threads on the exact final source with Clang `-O0`. The existing optimized builds also pass both native thread counts: operations uses exact final source, while the public build predates only the final schema description wording. This does not claim an exact-final optimized public metadata check.
