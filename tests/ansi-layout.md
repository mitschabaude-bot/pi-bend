...
A second review found the same eager-work pattern in `breakCells` and `wordState`: completed-line reversal/trimming and style continuation were prepared before the overflow selection. They now occur only inside its selected continuation. For hosted runs at N=1K/2K/4K, an unbroken 2N-character word at width N took 0.22/0.49/1.84 s before; N `x ` words at width N took 0.40/1.16/4.83 s. Afterward the respective 4K runs took 0.17/0.20 s and 10K runs 0.34/0.32 s. These bounded, startup-inclusive observations diagnose scaling, not universal timing guarantees. Both 20K-character wide-line cases now join the reference checks (seven long cases total).

Wide-line follow-up validation: all unchanged source/differential/Unicode/control checks and all seven long cases pass Bun and optimized native with explicit one/four threads. Exact artifacts are `build/ansi-layout-wide.js` and `build/ansi-layout-wide`, with logs `build/ansi-layout-wide-{bun,1,4}.log`.
