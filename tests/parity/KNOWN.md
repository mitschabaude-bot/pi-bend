...
- **Built-in provider catalog** (`print-bare-unauthed`, `print-bare-two-authed`):
  ...
  is ported). The native API-key `/login` flow now saves credentials for registered providers, including Cloudflare's key/account/gateway prompts. Bedrock resolves stored and ambient AWS credentials, but has no `/login` flow yet: its method selection needs select prompts, which the native login interaction does not offer.
