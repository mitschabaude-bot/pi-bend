...
- **Picker shrink position** (`scoped-models-selector`, `scoped-models-controls`, `settings-model-thinking*`, `settings-theme-marked`, `settings-theme-custom-name`): some interactions leave an extra blank row before the header. A full-height picker/filter/close fixture matches upstream main-screen writes and state; the cause in the complete terminal flow is still unresolved.
