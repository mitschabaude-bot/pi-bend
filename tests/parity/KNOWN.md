...
- **Built-in provider catalog** (`print-bare-unauthed`, `print-bare-two-authed`):
  ...
  dynamic catalog; its pi-messages API is ported). Google Vertex ADC accepts
  authorized_user credentials only; service-account files report that RSA
  signing is not available natively. The native API-key `/login` flow now saves credentials for registered providers, including Cloudflare's key/account/gateway prompts. Bedrock resolves stored and ambient AWS credentials and its provider login (bearer token, AWS profile or credential chain) is ported; its `/login` terminal flow has not been compared with pi yet.
