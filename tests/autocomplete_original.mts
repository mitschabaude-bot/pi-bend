...
const hashes={
 ...
 'src/fuzzy.ts':'0ae2bedc6a4f043d875ec415202a6e3d9e45405741359d7d2dc2855240dff633',
 'src/utils.ts':'8cda2d53e2361ac5aaf6d7345b2fee058c8df5743eae4e4e90c89a7072c026c3',
 'test/autocomplete.test.ts':'23d6aaa52acc11f9a36d667e6783ca2fb9041509aadacc127a9701768f93b40f',
 'test/autocomplete-skill-slash.test.ts':'627c312bde9a434b9a30bf567dea3640159b5c245f54fcd2a75dd8d54c55c728',
};
...
// Only the separator regexes are taken from utils.ts; the rest of that module is unrelated.
const utils=pinned('src/utils.ts'),regexStart=utils.indexOf('export const cjkBreakRegex'),regexEnd=utils.indexOf('function isPrintableAscii');
assert.ok(regexStart>0&&regexEnd>regexStart);
const separators=new Function(utils.slice(regexStart,regexEnd).replace(/^export /gm,'')+';return {autocompleteBoundaryRegex,autocompleteSeparatorRegex}')();
const Reference=new Function('spawn','readdirSync','statSync','homedir','basename','dirname','join','fuzzyFilter','autocompleteBoundaryRegex','autocompleteSeparatorRegex',strip('src/autocomplete.ts')+';return CombinedAutocompleteProvider')(spawn,fs.readdirSync,fs.statSync,os.homedir,path.basename,path.dirname,path.join,fuzzy,separators.autocompleteBoundaryRegex,separators.autocompleteSeparatorRegex);
...
// Native cursor columns count Unicode scalars; the source counts UTF-16 units.
const scalarColumn=(text,col)=>[...text.slice(0,col)].length;
const unitColumn=(text,col)=>[...text].slice(0,col).join('').length;
const nativeApplied=v=>v&&!v.error?{...v,cursorCol:unitColumn(v.lines[v.cursorLine],v.cursorCol)}:v;
const skillCommands=[
...
class Native {
 ...
 applyCompletion(lines,line,col,item,prefix){
  const actual=nativeApplied(invoke(['apply',lines.join('\n'),line,scalarColumn(lines[line]??'',col),item.value,item.label,prefix]));
...
const skillTests=[];
new Function('assert','describe','it','CombinedAutocompleteProvider',strip('test/autocomplete-skill-slash.test.ts'))(assert,(name,body)=>body(),(name,body)=>skillTests.push({name,body}),Native);
for(const t of skillTests){activeName=t.name;await t.body()}
assert.equal(skillTests.length,3);
...
let seed=19317;const alphabet='ab @"\t\'=./~，。\u3000查';
