"""HTTP connection candidate projection retains original reports, source occurrences and separate DNS families."""
import argparse
import errno
...
for suffix in ['c','js']:
 with (ROOT/f'build/http-addresses-{suffix}.log').open('w') as log:
  subprocess.run(['python3','scripts/run-rss-guarded.py','--limit-gib','8','--stats',f'build/http-addresses-{suffix}-build.json','--',str(bun),str(candidate/'main.ts'),'tests/http-addresses.bend','-o',f'build/http-addresses.{suffix}'],cwd=ROOT,check=True,stdout=log,stderr=subprocess.STDOUT)
audit='''
...
rows=[]
for backend,cmd in [('native 1',['build/http-addresses','--threads','1']),('native 4',['build/http-addresses','--threads','4']),('Bun',[str(bun),'build/http-addresses-audit.js'])]:
