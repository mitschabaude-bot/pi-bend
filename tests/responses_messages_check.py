...
def ablock(b):
    ...
    return 'T.AssistantToolCall{T.ToolCall{'+', '.join([string(b['id']),string(b['name']),record(b['arguments']),optional(b.get('thoughtSignature')),optional(b.get('namespace'))])+'}}'
def nativeTool(t):
...
def message(m):
    ...
    if m['role']=='assistant':return 'T.Assistant{Check.assistant('+', '.join([seq(map(ablock,m['content'])),string(m['api']),string(m['provider']),string(m['model']),'T.'+{'stop':'Stop','error':'Error','aborted':'Aborted'}[m['stopReason']]+'{}'])+')}'
...
for i,(c,r) in enumerate(zip(cases,expected,strict=True)):
    ...
    lines += [f'def case{i}() -> IO(Unit):','  Check.check('+', '.join([model,seq(map(message,c['messages'])),seq(map(string,c['allowed'])),options(c['options']),result,f'"Responses transcript {i}"'])+')']
groups=[]
