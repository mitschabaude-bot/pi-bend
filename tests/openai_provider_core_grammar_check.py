"""Real preparation grammar survives replacement; original hooks outlive core."""
import argparse,hashlib,json,re,socket,subprocess,threading
from pathlib import Path
from scoped_session_audit import prepare
ROOT=Path(__file__).resolve().parents[1]
p=argparse.ArgumentParser(description=__doc__)
p.add_argument('--prefix',type=Path,default=ROOT/'build/openai-provider-core-grammar')
...
if a.prepare_audit:
    prepare(prefix,ROOT/'build/bend-profiles/dns-transport-teles/bend2',any(b.startswith('native') for b in a.backends));raise SystemExit
...
for backend in a.backends:
    ...
    for mode in [0,14,15,2,3,30]:
        for supplied in [False,True]:
            expected_request=mode not in [2,30];errors=[];bodies=[];closed=[];stop=threading.Event()
            with socket.socket() as listener:
                ...
                try:run=subprocess.run(command+[str(mode),str(listener.getsockname()[1]),'yes' if supplied else 'no'],cwd=ROOT,capture_output=True,text=True,timeout=30)
            ...
            assert sorted(run.stderr.splitlines())==(['AUDIT 0 0 0','TIMERS 0 0'] if a.audit else []),run.stderr
            ...
            assert len([x for x in lines if x.startswith('payload:')])==(1 if mode==30 else 2),lines
    ...
    print(backend,'12 grammar/ownership cases PASS',flush=True)
pending=[ROOT/'tests/openai-provider-core-grammar.bend'];seen=set()
while pending:
    path=pending.pop().resolve()
    if path in seen:continue
    seen.add(path);pending.extend(path.parent/n for n in re.findall(r'^import (\.[^\s]+)',path.read_text(),re.M))
seen.update([Path(__file__).resolve(),ROOT/'tests/openai_provider_grammar_reference.mts',ROOT/'tests/responses_stream_reference.mts',ROOT/'tests/responses_prepare_reference.mts',ROOT/'tests/scoped_session_audit.py',ROOT/'tests/channel_audit.py'])
