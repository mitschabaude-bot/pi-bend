...
cd "$ROOT"
for suite in tool-call-parallel tool-call-parallel-gated tool-call-sequential validated-tool-execution tool-preparation-pipeline tool-execution tool-execution-stream agent-event-stream tool-finalization native-loop native-turn-hooks tool-preparation tool-emission truncated-calls assistant-snapshot; do
