...
cd "$ROOT"
for suite in agent agent-behavior agent-await agent-signal agent-busy agent-declarations agent-turn-options agent-continuation agent-late-updates agent-settings agent-input-callbacks agent-api-key agent-provider-callbacks agent-tool-hooks agent-turn-accessors agent-validation agent-events agent-listener-start agent-runtime-concurrency agent-lifecycle agent-loop agent-loop-tools agent-loop-arguments agent-loop-parallel agent-loop-steering agent-loop-turns agent-loop-turn-decisions agent-loop-termination tool-changes-clock turn-preparation turn-completion tool-execution-dispatch tool-call-parallel tool-call-parallel-gated tool-call-sequential validated-tool-execution tool-preparation-pipeline tool-execution tool-execution-stream agent-event-stream tool-finalization native-loop native-turn-hooks tool-preparation tool-emission truncated-calls assistant-snapshot; do
  flock /tmp/pi-bend-build.lock sh scripts/build-pure.sh "packages/agent/test/$suite.bend" "build/test-$suite"
...
node tests/agent_tool_hooks_reference.mts
node tests/agent_turn_accessors_reference.mts
node tests/agent_validation_reference.mts
