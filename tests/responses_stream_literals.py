...
def item(v):
    kind=v['type']
    if kind=='reasoning':return 'D.ReasoningItem{'+record(v)+'}'
    if kind=='message':
        phase='None{}' if v.get('phase') is None else 'Some{T.'+{'commentary':'Commentary','final_answer':'FinalAnswer'}[v['phase']]+'{}}'
        return 'D.MessageItem{'+string(v['id'])+', '+phase+', '+seq(string(x.get('text',x.get('refusal',''))) for x in v['content'])+'}'
    if kind in ['function_call','custom_tool_call']:
        args=string(v.get('arguments','')) if kind=='function_call' else optional(v.get('input'))
        return 'D.'+('FunctionItem' if kind=='function_call' else 'CustomItem')+'{'+', '.join([string(v['id']),string(v['call_id']),string(v['name']),args,optional(v.get('namespace'))])+'}'
    return 'D.OtherItem{}'
def event_literal(e):
    kind=e['type'].removeprefix('response.')
    if kind=='output_item.done':return 'D.ItemDone{'+item(e['item'])+'}'
    if kind=='reasoning_summary_part.done':return 'D.SummaryPartDone{}'
    ctor,field={
