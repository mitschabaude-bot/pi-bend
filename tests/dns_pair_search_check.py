"""Live paired candidate search: shared cursor/deadline, report retention and resource retirement."""
import argparse
import errno
...
for suffix in ['c','js']:
 with (ROOT/f'build/dns-pair-search-{suffix}.log').open('w') as log:
  subprocess.run(['python3','scripts/run-rss-guarded.py','--limit-gib','8','--stats',f'build/dns-pair-search-{suffix}-build.json','--',str(bun),str(candidate/'main.ts'),'tests/dns-pair-search.bend','-o',f'build/dns-pair-search.{suffix}'],cwd=ROOT,check=True,stdout=log,stderr=subprocess.STDOUT)
audit='''
...
'''
(ROOT/'build/dns-pair-search-audit.c').write_text((ROOT/'build/dns-pair-search.c').read_text()+audit)
subprocess.run(['flock', '/tmp/pi-bend-build.lock', 'clang','-std=c11','-fbracket-depth=2048','-O1','build/dns-pair-search-audit.c','-lpthread','-lm','-o','build/dns-pair-search'],cwd=ROOT,check=True)
(ROOT/'build/dns-pair-search-audit.js').write_text(instrument((ROOT/'build/dns-pair-search.js').read_text()))
...
modes=['suffix','partial4','partial6','aliases','forced','edns','priority','exhausted','terminal','deadline','error-second-candidate','absolute','zero','no-servers']
rows=[]
for backend,cmd in [('native 1',['build/dns-pair-search','--threads','1']),('native 4',['build/dns-pair-search','--threads','4']),('Bun',[str(bun),'build/dns-pair-search-audit.js'])]:
  ...
  for priority in [4,6]:
   for mode in modes:
    ...
    try:
     ...
     names=[] if mode in ['zero','no-servers'] else ['a'] if mode=='absolute' else ['a.x'] if mode in ['partial4','partial6','terminal','error-second-candidate'] else ['a.x','a.y','a'] if mode=='exhausted' else ['a.x','a'] if mode=='priority' and priority==4 else ['a.x','a.y']
     ...
     def respond(item):
      ...
      code=3 if owner=='a.x' or mode=='exhausted' else 0
     ...
     while process.poll() is None:
      ...
      for peer in ready:
       ...
       if owner in names:assert ident==2*names.index(owner)+(kind==28),(mode,owner,ident)
     ...
     lines+=['deadline:'+('expiry' if mode=='deadline' else 'none'),'draws:'+str(draws),'next:'+str(0 if mode=='no-servers' else ports[rotations%2])]
...
imports(ROOT/'tests/dns-pair-search.bend')
paths=sorted(str(p.relative_to(ROOT)) for p in sources)+['tests/dns_pair_search_check.py','tests/channel_audit.py']
record=dict(scope=__doc__,runs=rows,candidate=str(candidate),sha256={p:hashlib.sha256((ROOT/p).read_bytes()).hexdigest() for p in paths},compiler_sha256={p:hashlib.sha256((candidate/p).read_bytes()).hexdigest() for p in ['main.ts','bend.ts','comp.ts','base.bend']},builds={s:json.loads((ROOT/f'build/dns-pair-search-{s}-build.json').read_text()) for s in ['c','js']})
(ROOT/'build/dns-pair-search-result.json').write_text(json.dumps(record,indent=2)+'\n')
