"""Native default clock and SSE policy through owned provider sessions, including retirement."""
import argparse,ast,hashlib,json,re,socket,subprocess,threading,os,tempfile,time
from pathlib import Path
from scoped_session_audit import prepare
ROOT=Path(__file__).resolve().parents[1]
p=argparse.ArgumentParser(description=__doc__)
p.add_argument('--prefix',type=Path,default=ROOT/'build/openai-system-defaults')
...
for backend in a.backends:
    ...
    for mode in [0,14,15,2,3,22,30,31,32]:
        for supplied in [False,True]:
            ...
            diagnostics=['Could not parse message into JSON: {broken','From chunk: ["data: {broken"]'] if mode==32 else []
            assert sorted(run.stderr.splitlines())==sorted(diagnostics+(['AUDIT 0 0 0','FILES 0','TIMERS 0 0'] if a.audit else [])),run.stderr
            ...
            if mode==32:assert 'run:json:cleanup:none:delivery:none' in lines and 'diagnostic' not in lines and 'abort-hook' not in lines,lines
            stamps=[json.loads(x[len('timestamp:'):]) for x in lines if x.startswith('timestamp:')]
            assert len(stamps)==1+len([x for x in lines if x.startswith('event:')]),lines
            assert len(set(stamps))==1,stamps
            if supplied:
                assert stamps[0]==123 and lines.count('clock:sampled')==1 and lines[0]=='clock:sampled',lines
            else:
                assert started-1<=stamps[0]<=ended+1 and 'clock:sampled' not in lines,(started,stamps,ended,lines)
            runs.append(dict(timestamps=stamps,clock_bounds=[started,ended],stderr=run.stderr,backend=backend,audited=a.audit,mode=mode,supplied=supplied,trace=lines,requests=bodies,closed=closed))
    print(backend,'18 system defaults cases PASS',flush=True)
fixture.cleanup()
pending=[ROOT/'tests/openai-system-defaults.bend'];seen=set()
while pending:
