...
`\\p{N}`, `\\p{P}` and `\\p{S}` (Bun's regular expressions, a test-only oracle) over every
...
oracle = 'const out=[];let previous=99;for(let cp=0;cp<0x110000;cp++){const s=String.fromCodePoint(cp);const v=/\\p{L}/u.test(s)?1:/\\p{N}/u.test(s)?2:/\\p{P}/u.test(s)?3:/\\p{S}/u.test(s)?4:0;if(v!==previous){out.push(`${cp}:${v}`);previous=v}}console.log(out.join("\\n"))'
