...
- `createHarness({ resourceLoader })` is `createHarnessResources` with the skills and prompt templates as `Resources`; upstream's `createSyntheticSourceInfo` values are `Skills.SourceInfo` records.
- agent-session-prompt: response factories that inspect the context become the printed `request` lines (`lastUser`, `roles`, serialized `context`); `toolRuns` are `tool_run` lines of the `Runs` tool (the slow tool sleeps 25 ms). `preserves image attachments` sends a real 1x1 PNG through the image pipeline instead of mocking `processImage`. `extension sendUserMessage can opt into extension command dispatch` waits until the command has run, as upstream awaits `commandRun`.
- before_agent_start handlers return the options they want (`BeforeAgentStartEventResult.systemPromptOptions`) instead of editing `event.systemPromptOptions` in place. Request lines carry each system message's sections (`systemSections`) for #9789's section check. `uses the model selected by before_agent_start` checks the stored image's width (the strict model's 1000 px cap) with real image processing instead of the mocked `processImage` call.
