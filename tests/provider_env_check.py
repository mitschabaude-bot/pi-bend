...
if '--no-build' not in sys.argv:
    subprocess.run(['flock', '/tmp/pi-bend-build.lock', sys.executable,'scripts/run-rss-guarded.py','--stats','build/provider-env-build.json','--','sh','scripts/build-pure.sh','packages/ai/test/provider-env.bend','build/provider-env'],cwd=ROOT,check=True)
