...
`runtime/word-segmenter.bend` combines ICU 78.3's exact default word-rule automaton with the native weighted CJK engine and Khmer/Lao/Thai/Myanmar dictionary engines. It returns `Segment{text,isWordLike,status}` in source order; status preserves ICU's numeric rule status (0, 100, 200 or 400), and `isWordLike` follows the word-status interval. Mixed Latin, numbers, punctuation, Hebrew, emoji, CJK and Hangul retain their ICU boundaries and lexical classification. Pure UAX default segmentation remains available independently in `word-break.bend`; it is not substituted for ICU's tailored outer rules.

This loaded context supplies every dictionary engine selected by the pinned reference runtime. A one-scalar rule region needs no dictionary refinement and follows its rule result, as ICU does. Other scripts that ICU deliberately leaves to its unhandled-script behavior retain the outer rule boundaries. Phrase segmentation is outside this word-navigation interface. The reference has no packaged Thai/Myanmar LSTM models, so those languages use ICU's dictionary fallback; [sea-word-break.md](sea-word-break.md) records the verified resource-driven selection.

## Explicit immutable context

Load the six production assets once using the existing filesystem interface, decode them with `word-rules.decode`, `cjk-dictionary.decode`, `sea-word-data.decodeKhmer`, `decodeLao`, `decodeThai` and `decodeMyanmar`, and call `WordSegmenter.create(rules,cjkDictionary,khmerDictionary,laoDictionary,thaiDictionary,myanmarDictionary)`. The resulting `Context` owns immutable rule tables, the shared weighted CJK dictionary, Southeast Asian dictionaries and native normalization tables. `WordSegmenter.segments(context,text)` returns `Result<MissingEngine | InvalidState,List<Segment>>`; the latter error guards internal partition/progress invariants. No host code participates in production segmentation.

Interactive callers do not decode all six assets up front. A `Source` starts `Pending`, `ready` loads only the 15,884-byte rule table, and `readyFor(source,text)` also loads the dictionaries of the scripts that `text` contains: CJK/Hangul, Thai, Lao, Myanmar or Khmer, as routed by the rules. The first word command on Latin text therefore costs one small table, not the 4.3 MB of dictionaries. `word-navigation.sourceSegments` reports `WordDataPending` for text that needs a dictionary that is not loaded yet, so callers ready the source for that text and retry. `tests/word-lazy-load.bend` checks that lazily loaded segments equal the fully loaded context's for each script and for mixed text.
...
The forward machine retains the most recent accepting boundary and its original suffix, allowing punctuation lookahead without copying or repeatedly reversing entire prefixes. The accepted rule region then drives dictionary dispatch. Contiguous CJK/Hangul and Southeast Asian engine ranges are refined inside that region; the engine's terminal endpoint is omitted, then the outer rule endpoint is retained. This preserves ICU's attachment of intervening marks and non-dictionary prefixes/suffixes. Every subdivided piece receives the parent rule's final status, matching ICU's dictionary cache. All reference engines are supplied explicitly by the loaded context.
...
`tests/word_segmenter_check.py` passes 214,511 exact results on Bun and native one/four threads. It compares every segment string, lexical flag and numeric status against Node's ICU 78.3 engine and separately confirms that ICU's strings/flags agree with `Intl.Segmenter`. Coverage includes the 1,944 official Unicode word-boundary inputs (against ICU's actual tailoring), exhaustive triples of 12 mixed word/mark/punctuation/emoji units, both ends of every generated scalar-property range, surrounding Latin/numeric/CJK contexts, 1,024 random mixed-language strings, all 81,028 Khmer dictionary entries, 2,048 Khmer word concatenations, 2,048 unknown Khmer runs, 512 mixed Khmer/CJK/Latin inputs, all 30,550 Lao dictionary entries, 2,048 Lao word concatenations, 2,048 unknown Lao runs, 512 mixed Lao/Khmer/CJK/Latin inputs, all 26,383 Thai and 41,120 Myanmar dictionary entries plus 2,048 concatenations, 2,048 unknown runs and 512 mixed contexts for each, 300 exhaustive Thai suffix combinations, and twenty-seven long/mark inputs including 100,000-letter and 100,000-mark runs. The first case exercises Common marks before any earlier native text can initialize hidden state.

The expanded passing corpus took about 64.293 seconds on Bun and 6.125/6.750 seconds on native one/four threads, including dictionary loading and output. These are observations from a shared machine, not controlled throughput benchmarks. Generic maximal-rule-match, partition/status and dictionary-dispatch laws remain unproved; these differential checks are not presented as proofs.
...
The Southeast Asian data, exact lookahead/resynchronization algorithm and full prefix comparison are documented in [sea-word-break.md](sea-word-break.md). Generic partition/progress proofs remain unproved; LSTM-enabled ICU distributions are a different engine policy from this pinned reference.
