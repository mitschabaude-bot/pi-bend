...
report = dict(source_closure_sha256=source_hashes, independent_byte_parser_checks=parser_checks, cases=len(rows), backends=['native 1', 'native 4', 'Bun'], comparisons=3*len(rows), oracle='Node URLSearchParams with literal non-ASCII converted to UTF-8 percent bytes', node_version=subprocess.check_output(['node','--version'], text=True).strip(), raw_node_differences=oracle_differences, differences=differences, sha256={p:digest(p) for p in ['packages/runtime/src/url-search-params.bend','packages/runtime/src/url-form.bend','packages/runtime/test/url-search-params.bend','tests/url_search_params_check.py','build/url-search-params','build/url-search-params.js']})
