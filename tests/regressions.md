...
- Chunks are a fixed four tokens (16 characters) instead of a random 3-5 tokens. Usage follows upstream's `withUsageEstimate`: the output is `ceil(characters / 4)` of the reply, and the prompt is the serialized request context (`serializeContext`). As in upstream's harness, the agent has no session id, so the faux's prompt-cache split never applies. Replies carry the request's model, as upstream's faux clones them with it. Upstream's harness wires the built-in tools; the 7925 tool call runs against no tools and gets an error result, which the assertions don't cover.
