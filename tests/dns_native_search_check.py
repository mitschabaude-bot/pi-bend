"""Configured transport composed with search candidates, aliases and total deadlines."""
import argparse
import hashlib
import json
import re
...
for suffix in ['c','js']:
    ...
    with (ROOT/f'build/dns-native-search-{suffix}.log').open('w') as log:
        subprocess.run(['python3','scripts/run-rss-guarded.py','--limit-gib','8','--stats',f'build/dns-native-search-{suffix}-build.json','--',str(bun),str(candidate/'main.ts'),'tests/dns-native-search.bend','-o',f'build/dns-native-search.{suffix}'],cwd=ROOT,check=True,stdout=log,stderr=subprocess.STDOUT)
source=(ROOT/'build/dns-native-search.c').read_text()+'''
...
'''
(ROOT/'build/dns-native-search-audit.c').write_text(source)
subprocess.run(['flock', '/tmp/pi-bend-build.lock', 'clang','-std=c11','-fbracket-depth=2048','-O1','build/dns-native-search-audit.c','-lpthread','-lm','-o','build/dns-native-search'],cwd=ROOT,check=True)
(ROOT/'build/dns-native-search-audit.js').write_text('process.on("exit",()=>console.error(`AUDIT ${globalThis.BEND_IO.live} ${globalThis.BEND_IO.waits.length}`));\n'+(ROOT/'build/dns-native-search.js').read_text())
...
cases=[
 ...
 ('size',[],'zero'),('unsupported',[],'type'),('zero-attempts',[],'transport'),
]
rows=[]
for backend,command in [('native 1',['build/dns-native-search','--threads','1']),('native 4',['build/dns-native-search','--threads','4']),('Bun',[str(bun),'build/dns-native-search-audit.js'])]:
  ...
  for kind in [1,28]:
   for mode,plan,answer in cases:
    ...
    try:
     ...
     draws=0 if mode in ['pre','size','unsupported'] else 1 if mode in ['error-first','zero-attempts','connection-refused'] else len(owners)+int(mode=='error-second')
     expected=answer if answer.startswith(('dns:','entropy:','abort:')) or answer in ['transport','zero','type'] else selected(answer,kind)
     reason='expiry' if mode=='deadline' else 'parent' if mode=='pre' else 'none'
     lines=[expected,reason,f'next:{ports[0]}',f'draws:{draws}']
...
imports(ROOT/'tests/dns-native-search.bend')
paths=sorted(str(p.relative_to(ROOT)) for p in sources)+['tests/dns_native_search_check.py']
record=dict(scope=__doc__,runs=rows,candidate=str(candidate),sha256={p:hashlib.sha256((ROOT/p).read_bytes()).hexdigest() for p in paths},compiler_sha256={p:hashlib.sha256((candidate/p).read_bytes()).hexdigest() for p in ['main.ts','bend.ts','comp.ts','base.bend']},builds={s:json.loads((ROOT/f'build/dns-native-search-{s}-build.json').read_text()) for s in ['c','js']})
(ROOT/'build/dns-native-search-result.json').write_text(json.dumps(record,indent=2)+'\n')
