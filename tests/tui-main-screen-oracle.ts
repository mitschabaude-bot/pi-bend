...
frame(ids, ["plain"], 21);
const picker = setup();
const numbered = (count: number, start = 0) => Array.from({ length: count }, (_, i) => String(start + i));
const pickerLines = (lines: string[], cursorColumn: number) => lines.map((line, i) => i === 12 ? line.slice(0, cursorColumn) + mark + line.slice(cursorColumn) : line);
frame(picker, pickerLines(numbered(16), 2), 100, 32);
frame(picker, pickerLines(numbered(32), 2), 100, 32);
frame(picker, pickerLines([...numbered(13), "filtered", ...numbered(13, 14)], 2), 100, 32);
frame(picker, pickerLines(numbered(16), 2), 100, 32);
