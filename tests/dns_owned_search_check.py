"""Persistent native resolver owners execute configured search and repeated lookups."""
import argparse
import hashlib
import json
import re
...
for suffix in ['c','js']:
    ...
    with (ROOT/f'build/dns-owned-search-{suffix}.log').open('w') as log:
        subprocess.run(['python3','scripts/run-rss-guarded.py','--limit-gib','8','--stats',f'build/dns-owned-search-{suffix}-build.json','--',str(bun),str(candidate/'main.ts'),'tests/dns-owned-search.bend','-o',f'build/dns-owned-search.{suffix}'],cwd=ROOT,check=True,stdout=log,stderr=subprocess.STDOUT)
source=(ROOT/'build/dns-owned-search.c').read_text()+'''
...
'''
(ROOT/'build/dns-owned-search-audit.c').write_text(source)
subprocess.run(['flock', '/tmp/pi-bend-build.lock', 'clang','-std=c11','-fbracket-depth=2048','-O1','build/dns-owned-search-audit.c','-lpthread','-lm','-o','build/dns-owned-search'],cwd=ROOT,check=True)
(ROOT/'build/dns-owned-search-audit.js').write_text('process.on("exit",()=>console.error(`AUDIT ${globalThis.BEND_IO.live} ${globalThis.BEND_IO.waits.length}`));\n'+(ROOT/'build/dns-owned-search.js').read_text())
...
rows=[]
for backend,command in [('native 1',['build/dns-owned-search','--threads','1']),('native 4',['build/dns-owned-search','--threads','4']),('Bun',[str(bun),'build/dns-owned-search-audit.js'])]:
...
imports(ROOT/'tests/dns-owned-search.bend')
paths=sorted(str(p.relative_to(ROOT)) for p in sources)+['tests/dns_owned_search_check.py']
record=dict(scope=__doc__,runs=rows,candidate=str(candidate),sha256={p:hashlib.sha256((ROOT/p).read_bytes()).hexdigest() for p in paths},compiler_sha256={p:hashlib.sha256((candidate/p).read_bytes()).hexdigest() for p in ['main.ts','bend.ts','comp.ts','base.bend']},builds={s:json.loads((ROOT/f'build/dns-owned-search-{s}-build.json').read_text()) for s in ['c','js']})
(ROOT/'build/dns-owned-search-result.json').write_text(json.dumps(record,indent=2)+'\n')
