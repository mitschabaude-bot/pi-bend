...
fixtures=[dict(initial=a,later=b,stop=s,fail=f) for a,b,s,f in itertools.product(range(4),range(4),[False,True],[False,True])]
...
for f,value in zip(fixtures,expected):lines.append(f'    T.scenario({f["initial"]}, {f["later"]}, {"True{}" if f["stop"] else "False{}"}, {"True{}" if f["fail"] else "False{}"}, {json.dumps(value)})')
lines.append('    IO.print("PASS 64 Agent hook capture/precedence/signal source scenarios")')
(root/'build').mkdir(exist_ok=True)
(root/'build/agent-hooks-check.bend').write_text('\n'.join(lines)+'\n')
subprocess.run(['sh','scripts/build-pure.sh','build/agent-hooks-check.bend','build/agent-hooks-check'],cwd=root,check=True)
