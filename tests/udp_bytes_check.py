...
bun=Path.home()/'.bun/bin/bun';candidate=ROOT/'build/bend-udp-bytes-candidate'
for suffix in ['c','js']:
...
for backend,cmd in [('native 1',['build/udp-bytes','--threads','1']),('native 4',['build/udp-bytes','--threads','4']),('Bun',[str(bun),'build/udp-bytes.js'])]:
    for maximum,data in cases:
        with socket.socket(socket.AF_INET,socket.SOCK_DGRAM) as peer, socket.socket(socket.AF_INET,socket.SOCK_DGRAM) as second:
            peer.bind(('127.0.0.1',0));second.bind(('127.0.0.1',0));limits=[maximum,65535] if maximum<=65535 else [maximum,65535,65535]
            p=subprocess.Popen([*cmd,*map(str,limits)],cwd=ROOT,stdout=subprocess.PIPE,stderr=subprocess.PIPE,text=True)
            try:
                assert select.select([p.stdout],[],[],8)[0], 'bind timed out'
                local=p.stdout.readline().strip().split(',');assert local[:5]==['4','0','0','0','0'],local
                peer.sendto(data,('127.0.0.1',int(local[5])))
                second.sendto(b'\0\xff',('127.0.0.1',int(local[5])))
                out,err=p.communicate(timeout=12)
                bound=min(maximum,65535);expected=f'4:2130706433:0:0:0:{peer.getsockname()[1]}:0:{int(len(data)>bound)}:'+''.join(str(b)+',' for b in data[:bound])
                want=([expected] if maximum<=65535 else ['error:22',expected])+[f'4:2130706433:0:0:0:{second.getsockname()[1]}:0:0:0,255,']
                assert p.returncode==0 and not err and out.splitlines()==want,(backend,maximum,p.returncode,err,out[:300])
                rows.append(dict(backend=backend,maximum=maximum,bytes=len(data),truncated=len(data)>bound))
            finally:
                if p.poll() is None:p.kill();p.wait()
    print(f'{backend}: {len(cases)} binary UDP cases PASS',flush=True)
