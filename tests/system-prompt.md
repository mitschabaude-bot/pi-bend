...
`core/skills.bend` contains the shared typed `Skill` and `SourceInfo` data and exact `formatSkillsForPrompt` behavior, including XML escaping, disabled-skill filtering, list order, and read/bash instructions. Its `FileReadTool` argument is explicit; `Read{}` is the upstream default. Discovery, frontmatter parsing, loading and diagnostics are now implemented and validated separately in [skills.md](skills.md). Package metadata conversion and broader source-info factory helpers remain separate resource-loader work. SDK/session prompt-update integration, extension hooks, and tool snippet declarations are not claimed by these pure tests.
