"""Concurrent read/write ownership with a deliberately blocked upload."""
...
def check(command, label):
    ...
    with socket.socket() as listener, concurrent.futures.ThreadPoolExecutor(max_workers=2) as pool:
            ...
            for index in range(10):
                with listener.accept()[0] as peer:
                    ...
                    if index == 0:
                        ...
                        # A connection can reuse descriptor numbers; associate
                        # its observed park with the ordinal, not the raw fd.
                        with condition:
                            assert condition.wait_for(lambda: index in parked, timeout=15), 'uploader never parked'
                        peer.sendall(b'R')
                        # Reading only after cancellation keeps backpressure in
                        # place while the client observes the early response.
                        with condition:
                            assert condition.wait_for(lambda: any(r == ['ABORT', str(index)] for r in rows), timeout=15), 'client never aborted'
                        data = bytearray()
                        while chunk := peer.recv(4096):
                            data.extend(chunk)
                        assert len(data) < 65536, 'whole upload completed'
                        assert data == (bytes(range(256)) * 256)[:len(data)], 'wrong upload prefix'
        ...
        try:
            ...
            assert output == 'PASS socket writer\n', output
        finally:
        ...
        for row in rows:
            group.append(row)
            if sum(r[0] == 'CLOSE' for r in group) == (2 if len(groups) == 9 else 3):
                groups.append(group)
                group = []
        assert len(groups) == 10 and not group, groups
        for index, group in enumerate(groups):
            duplicates = [r for r in group if r[0] == 'DUP']
            assert len(duplicates) == (1 if index == 9 else 2), group
            ...
            assert len([r for r in group if r[0] == 'SHUT']) == (0 if index == 0 else 1), group
            if 1 <= index <= 8:
                assert ['WRITE', duplicates[1][2]] in group, group
        print(label + ': 8 early replies during parked uploads; shared abort; normal/pre-abort ownership; 29 descriptors closed PASS', flush=True)


with tempfile.TemporaryDirectory(prefix='socket-writer-', dir=ROOT / 'build') as temporary:
