...
launcher = ROOT / 'build/http-socket-exchange-compiler'
...
binary = ROOT / 'build/http-socket-exchange'
subprocess.run(['flock', '/tmp/pi-bend-build.lock', 'sh', 'scripts/build-pure.sh', 'tests/http-socket-exchange.bend', str(binary)], cwd=ROOT, env=environment, check=True)
javascript = binary.with_suffix('.js')
subprocess.run([str(launcher), 'tests/http-socket-exchange.bend', '-o', str(javascript)], cwd=ROOT, check=True)
    ...
    with socket.socket() as listener, concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
        ...
        def server():
            for index in range(24):
                with listener.accept()[0] as peer:
                    ...
                    mode = index % 8
                    if mode in [6, 7]:
                        assert peer.recv(1) == b'', 'rejected start sent request bytes'
                        continue
                    request = b''
                    while b'\r\n\r\n' not in request:
        ...
        for index, line in enumerate(lines):
            ...
            if mode in [0, 1, 2]:
                ...
                assert line == 'start-abort:stop', (index, line)
        assert not result.stderr, result.stderr
    print(label + ': 24 owned exchange lifecycle traces PASS', flush=True)
