...
The integrated root build also passes all seven scenarios on explicit native one/four workers after the late-output callback lifetime correction (`13be475`), using the private combined toolchain with the process TU patch. This validates the final assembly, beyond the original isolated fixture build.
