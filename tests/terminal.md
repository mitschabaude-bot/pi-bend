...
patch -p1 -d build/bend-terminal-toolchain < patches/bend-process-clock.patch
...
Linux TTYs, FIFOs and regular redirected files are implemented; socket-backed descriptors and other platform terminal backends are not claimed. Windows VT setup and native macOS modifier-key sensing remain pending; the pure normalization helpers are implemented and replayed, but platform sensing is not wired into this Linux driver. Directory-valued `PI_TUI_WRITE_LOG` creates `tui-YYYY-MM-DD_HH-mm-ss-PID.log` using local civil time and the current process ID, including through symlinked directories. Filename formatting and path selection live in Bend; `patches/bend-process-clock.patch` supplies only the OS process-ID and local-time effects. Stat or local-time failure retains the configured path, so an attempted write reports its typed filesystem error instead of silently disabling logging. Full terminal/TUI parity, including those platform paths and higher-level rendering, remains partial.

Integration validation of the owned-output fix rebuilt the native fixtures from an isolated copy of `build/bend-process-files` with the updated terminal patch. All 32 PTY scenarios pass on one/four threads, including original descriptor close/reuse and stale/wrong-kind capability rejection. The rebuilt pure protocol fixture also passes all 90 negotiation traces and 19 original helper calls on Bun/native one/four; no shared compiler changes were needed.

Directory-log integration passes all 35 PTY/file scenarios on native one/four, including UTC, Pacific/Kiritimati and America/New_York filename/PID checks, symlinked directories, exact-path append and failed log opens. The rebuilt protocol fixture still passes all 90 traces and 19 helper calls on Bun/native one/four. Validation used the isolated terminal-plus-process-clock candidate; the shared compiler was not modified.
