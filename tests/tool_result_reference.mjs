...
for (let content = 0; content < 3; content++) for (let details = 0; details < 6; details++) for (let flags = 0; flags < 6; flags++) {
  const originalDetails = {}, replacementDetails = {};
  ...
  cases.push({content, details, flags, expectedDetails, expectedContent:result.content.join(','),
    expectedUsage:result.usage === newUsage, expectedTerminate:result.terminate ?? null,
    expectedError:outcome.isError, same:result === original});
}
const original = {content:['old'], details:{}, terminate:true};
if (apply(original, false, undefined).result !== original) throw new Error('absent hook must preserve original');
