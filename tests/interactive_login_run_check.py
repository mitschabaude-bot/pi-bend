    ...
    with ThreadingHTTPServer(("127.0.0.1", 0), Handler) as server:
        ...
        with tempfile.TemporaryDirectory(prefix="pi-interactive-login-") as place:
            ...
            try:
                ...
                back = len(output)
                os.write(master, b"\x1b")
                until(b"Select authentication method:", back)
                assert b"Login cancelled" not in output[back:]
                back = len(output)
                os.write(master, b"\r")
                ...
                with socket.socket() as occupied:
                    ...
                    until(b"Select authentication method for Anthropic:", at)
                    os.write(master, b"\r")
                    until(b"Complete login in your browser, or paste", at)
                ...
                until(b"Select authentication method for Anthropic:", at)
                os.write(master, b"\x1b[B\r")
                until(b"Enter Anthropic API key", at)
                os.write(master, b"sk-anthropic-test\r")
                until(b"Saved API key for Anthropic", at)
                saved = json.loads((agent_dir / "auth.json").read_text())["anthropic"]
                assert saved["type"] == "api_key" and saved["key"] == "sk-anthropic-test", saved
                at = len(output)
                os.write(master, b"/logout\r")
                until(b"Stored credentials", at)
                os.write(master, b"anthropic\r")
                until(b"Removed stored API key for Anthropic", at)
                assert "anthropic" not in json.loads((agent_dir / "auth.json").read_text())
                at = len(output)
                os.write(master, b"/logout\r")
                until(b"Stored credentials", at)
                os.write(master, b"\x1b")
                ...
                until(b"Select provider to configure", at)
                until(b"No matching providers", at)
                os.write(master, b"\x1b")
                time.sleep(.3)
                assert b"Unknown login provider" not in output[at:]
                at = len(output)
                ...
                os.close(slave)
        server.shutdown()
    print(f"native{threads}: model-free Codex browser/device and Anthropic browser/manual login, cancellation, saved credentials, model selection, API-key method selection and logout")
