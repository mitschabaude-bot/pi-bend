"""Live HTTP resolution into ordered connection dispatch with an injected affine connector; no synthetic destination is contacted."""
import argparse
import errno
...
for suffix in ['c','js']:
 with (ROOT/f'build/http-connect-{suffix}.log').open('w') as log:
  subprocess.run(['python3','scripts/run-rss-guarded.py','--limit-gib','8','--stats',f'build/http-connect-{suffix}-build.json','--',str(bun),str(candidate/'main.ts'),'tests/http-connect.bend','-o',f'build/http-connect.{suffix}'],cwd=ROOT,check=True,stdout=log,stderr=subprocess.STDOUT)
audit='''
...
'''
(ROOT/'build/http-connect-audit.c').write_text((ROOT/'build/http-connect.c').read_text()+audit)
subprocess.run(['flock', '/tmp/pi-bend-build.lock', 'clang','-std=c11','-fbracket-depth=2048','-O1','build/http-connect-audit.c','-lpthread','-lm','-o','build/http-connect'],cwd=ROOT,check=True)
(ROOT/'build/http-connect-audit.js').write_text(instrument((ROOT/'build/http-connect.js').read_text()))
...
modes=['suffix','partial4','partial6','aliases','forced','edns','priority','exhausted','terminal','deadline','error-second-candidate','absolute','zero','local','local-invalid-name','local-v4','local-v6','source-error','invalid-name','repeat','numeric4','numeric6','numeric-mapped','empty-host','empty-domain','opaque-host']
...
local.update({'numeric4':'numeric4:2130706433','numeric6':'numeric6:0,0,0,1:0','numeric-mapped':'numeric6:0,0,65535,2130706433:0','empty-host':'invalid-http-host','empty-domain':'invalid-http-host','opaque-host':'invalid-http-host'})
rows=[]
for backend,cmd in [('native 1',['build/http-connect','--threads','1']),('native 4',['build/http-connect','--threads','4']),('Bun',[str(bun),'build/http-connect-audit.js'])]:
  ...
  for priority in [4,6]:
   for mode in modes:
    ...
    try:
     ...
     if mode in ['local','local-invalid-name','local-v4','local-v6']:
      entries=local[mode][len('local:'):].split(';')[:-1]
      projected='ordered:'+''.join(('4,'+entry.split('/')[0][2:]+';' if entry.startswith('4:') else '6,'+entry.split('/')[0][2:]+',0;') for entry in entries)
     elif mode=='numeric4':projected='ordered:4,2130706433;'
     elif mode=='numeric6':projected='ordered:6,0,0,0,1,0;'
     elif mode=='numeric-mapped':projected='ordered:6,0,0,65535,2130706433,0;'
     elif mode in local or mode in ['zero','error-second-candidate','exhausted','terminal','deadline']:projected='no-addresses'
     else:projected='paired:'+('' if mode=='partial6' else '4,16909060;')+'/'+('' if mode=='partial4' else '6,0,0,0,1,0;')
     if projected=='no-addresses':lines.append(projected)
     else:
      raw=projected.split(':',1)[1]
      if projected.startswith('ordered:'):
       addresses=raw.split(';')[:-1]
       preferred=addresses[0][0]
      else:
       left,right=raw.split('/')
       addresses=left.split(';')[:-1]+right.split(';')[:-1]
       preferred='6' if priority==4 else '4'
      groups={kind:list(dict.fromkeys(value for value in addresses if value.startswith(kind+','))) for kind in ['4','6']}
      other='6' if preferred=='4' else '4';ordered=[]
      for index in range(max(map(len,groups.values()))):
       for kind in [preferred,other]:
        if index<len(groups[kind]):ordered.append(groups[kind][index])
      lines=['connect:'+''.join(value+';' for value in ordered)]+lines+['connected']
...
imports(ROOT/'tests/http-connect.bend')
paths=sorted(str(p.relative_to(ROOT)) for p in sources)+['tests/http_connect_check.py','tests/channel_audit.py']
record=dict(scope=__doc__,runs=rows,candidate=str(candidate),sha256={p:hashlib.sha256((ROOT/p).read_bytes()).hexdigest() for p in paths},compiler_sha256={p:hashlib.sha256((candidate/p).read_bytes()).hexdigest() for p in ['main.ts','bend.ts','comp.ts','base.bend']},builds={s:json.loads((ROOT/f'build/http-connect-{s}-build.json').read_text()) for s in ['c','js']})
(ROOT/'build/http-connect-result.json').write_text(json.dumps(record,indent=2)+'\n')
