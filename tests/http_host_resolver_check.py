"""Typed HTTP host routing: numeric bypass plus complete live hosts and paired DNS resolution."""
import argparse
import errno
...
for suffix in ['c','js']:
 with (ROOT/f'build/http-host-resolver-{suffix}.log').open('w') as log:
  subprocess.run(['python3','scripts/run-rss-guarded.py','--limit-gib','8','--stats',f'build/http-host-resolver-{suffix}-build.json','--',str(bun),str(candidate/'main.ts'),'tests/http-host-resolver.bend','-o',f'build/http-host-resolver.{suffix}'],cwd=ROOT,check=True,stdout=log,stderr=subprocess.STDOUT)
audit='''
...
'''
(ROOT/'build/http-host-resolver-audit.c').write_text((ROOT/'build/http-host-resolver.c').read_text()+audit)
subprocess.run(['flock', '/tmp/pi-bend-build.lock', 'clang','-std=c11','-fbracket-depth=2048','-O1','build/http-host-resolver-audit.c','-lpthread','-lm','-o','build/http-host-resolver'],cwd=ROOT,check=True)
(ROOT/'build/http-host-resolver-audit.js').write_text(instrument((ROOT/'build/http-host-resolver.js').read_text()))
...
rows=[]
for backend,cmd in [('native 1',['build/http-host-resolver','--threads','1']),('native 4',['build/http-host-resolver','--threads','4']),('Bun',[str(bun),'build/http-host-resolver-audit.js'])]:
...
imports(ROOT/'tests/http-host-resolver.bend')
paths=sorted(str(p.relative_to(ROOT)) for p in sources)+['tests/http_host_resolver_check.py','tests/channel_audit.py']
record=dict(scope=__doc__,runs=rows,candidate=str(candidate),sha256={p:hashlib.sha256((ROOT/p).read_bytes()).hexdigest() for p in paths},compiler_sha256={p:hashlib.sha256((candidate/p).read_bytes()).hexdigest() for p in ['main.ts','bend.ts','comp.ts','base.bend']},builds={s:json.loads((ROOT/f'build/http-host-resolver-{s}-build.json').read_text()) for s in ['c','js']})
(ROOT/'build/http-host-resolver-result.json').write_text(json.dumps(record,indent=2)+'\n')
