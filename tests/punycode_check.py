...
if '--no-build' not in sys.argv:
    subprocess.run(['flock', '/tmp/pi-bend-build.lock', 'sh', 'scripts/build-pure.sh', 'packages/runtime/test/punycode.bend', 'build/punycode'], cwd=ROOT, check=True)
subprocess.run([str(Path(BEND)), 'packages/runtime/test/punycode.bend', '-o', 'build/punycode.js'], cwd=ROOT, check=True)
