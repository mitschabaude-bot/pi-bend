"""DNS presentation conversion against libc ns_name_pton, without networking.

Includes absolute/relative status and complete wire bytes. Raw NUL and invalid
Bend byte values have additional typed-boundary checks, not a C-string oracle.
"""
import ctypes
import hashlib
import json
...
subprocess.run(['flock', '/tmp/pi-bend-build.lock', 'clang','-std=c11','-fbracket-depth=2048','-O1','build/dns-presentation.c','-lpthread','-lm','-o','build/dns-presentation'],cwd=ROOT,check=True)
...
def oracle(text):
    ...
    return str(absolute)+':'+''.join(str(byte)+',' for byte in raw[:end])
...
for label,command in [('native 1',['build/dns-presentation','--threads','1']),('native 4',['build/dns-presentation','--threads','4']),('Bun',[str(bun),'build/dns-presentation.js'])]:
    for start in range(0,len(values),64):
        batch=values[start:start+64];want=['invalid']*3+expected[start:start+64]
