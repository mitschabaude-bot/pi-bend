"""Check Responses callback dispatch and ownership with injected native/custom fetch."""
import hashlib
import json
...
fixture = root / 'packages/ai/test/openai-responses-fetch.bend'
expected = {
    'native': 'native:POST:{"stream":true}\nclosed\n',
    'custom': 'custom:POST:{"stream":true}\nclosed\n',
    'invalid': 'invalid-timeout\n',
    'pre': 'aborted\n',
}
results = []
with tempfile.TemporaryDirectory(dir=root / 'build', prefix='openai-responses-fetch-') as directory:
    folder = Path(directory)
    for suffix in ('c', 'js'):
        subprocess.run(['python3','scripts/run-rss-guarded.py','--limit-gib',('16' if suffix=='c' else '8'),'--stats',str(root/'build'/('openai-responses-fetch-'+suffix+'-build.json')),'--',str(bun), str(candidate / 'main.ts'), str(fixture), '-o', str(folder / ('run.' + suffix))], cwd=root, check=True)
    ...
    for backend, command in [('native-1', [str(folder / 'run'), '--threads', '1']),
        ...
        for mode in expected:
          for repetition in range(4):
            ...
            assert created == closed == (0 if mode in ('pre','invalid') else 1), result.stderr
...
sources = [fixture, root / 'packages/runtime/src/deadline.bend', root/'packages/runtime/src/http-fetch-scope.bend', root/'packages/runtime/src/http-response.bend', root/'tests/openai_responses_fetch_check.py', root/'packages/ai/src/api/openai-responses-fetch.bend', root/'packages/runtime/src/timer-milliseconds.bend',
           candidate / 'base.bend', candidate / 'effs/timer.c', candidate / 'effs/timer.js']
(root / 'build/openai-responses-fetch-result.json').write_text(json.dumps(dict(
    scope='Concrete Responses adapter: native fallback, custom override, uppercase POST and serialized body preservation, pre-abort and invalid timeout skip invocation, caller-owned callback disposal. 12 unmodified and 48 instrumented runs. Native audits timer/channel rows; Bun audits timers. Open-body lifecycle covered separately by http-fetch-scope.',
