...
The native build requires the process, null-stdin, and process-TU effect patches plus the filesystem primitives used by the canonical tool. Native one/four-thread runs pass 40 scenarios each, including both repeated-lifecycle cases. Bun passes all 22 injected-operation scenarios, including the unchanged 60,000-byte single-chunk case; native process spawning remains explicitly unsupported on that backend. The wire reporter calls the direct Base64 encoder on UTF-8 bytes, whose octet range is already guaranteed. The original reporter used `B64.encode`, whose recursive `octets` validator overflowed the Bun stack on this large output; this was fixture serialization, not Bash accumulation. These are integration regressions, not proofs or a claim that all upstream tools tests are ported. Other platform shells, concurrent active invocations and OS failure injection remain outside this fixture.
