...
for backend in ['bun','native-1','native-4']:
 ...
 assert len(parts)==8,(backend,parts)
 before,still_old,stage,published,release,press,hidden,journal=parts
 ...
 assert press==hidden==expected_frame+':11',(backend,press,hidden)
 assert journal=='R1/8,R2/8,R3/2,I3,F3-,F1+,I1,',(backend,journal)
 print(f'{backend}: frame commit and input callback sequence passed')
print('pending owner contract: dynamic visibility predicate refresh (source calls recorded)')
